...
const char* skill_cat_name [ MAX_SKILL_CAT ] = { "weapon", "language", "spell", "physical", "trade", "song", "ward" };
...
char* abil_name( int i )
{
    ...
    if( i = 2 ) return "(dreadful) ";
    if( i = 3 ) return "  (awful)  ";
    if( i = 4 ) return "   (bad)   ";
    if( i = 5 ) return "  (poor)   ";
    if( i = 6 ) return "  (fair)   ";
    if( i = 7 ) return " (capable) ";
    if( i = 8 ) return "  (good)   ";
    if( i = 9 ) return "(very good)";
    if( i = 10 ) return " (superb)  ";
    return " (wicked)  ";
}
...
void do_abilities( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        ...
        send( ch, "Where <cat> is one of Phy, Wea, Spe, Lan, War, or Son.\n\r" );
        ...
        for( j = 0; j < MAX_SKILL_CAT; j++ ) {
            if( !strncasecmp( skill_cat_name[j], arg, length ) ) {
                for( i = 0; i < MAX_SKILL; i++ ) {
                    if( skill_table[i].category != j || ( cost = skill_table[i].prac_cost[clss] ) == -1 || skill_table[i].level[clss] > ch->shdata->level )
                        continue;       
                    if( !found ) {
                        page_underlined( ch, "Skill             Level   Cost   Pracs    Prerequistes\n\r" );
                        found = TRUE;
	                }
                    ...
                    if( ch->shdata->skill[i] != 10 )
                        sprintf( buf+strlen(buf), "%8d%6d%6s", ch->shdata->level*cost*cost, cost, "" );
                    else
                    ...
                    if( ps2 != SKILL_NONE ) 
                        sprintf( buf+strlen(buf), " & %s [%d]", skill_table[ps2].name, skill_table[i].pre_level[1] );
    ...
    for( j = 0; j < MAX_SKILL_CAT; j++ ) {
        if( !strncasecmp( skill_cat_name[j], arg, length ) ) {
            for( i = 0; i < MAX_SKILL; i++ ) {
                if( skill_table[i].category != j || ( cost = skill_table[i].prac_cost[clss] ) == -1 )
                continue;   
                if( !found ) {
                    page_underlined( ch,  "Skill               Pracs  Level     Prerequistes\n\r" );
                    found = TRUE;
	            }
                ...
                if( ps1 != SKILL_NONE )
                    sprintf( buf+strlen(buf), "%s [%d]", skill_table[ps1].name,  skill_table[i].pre_level[0] );
                if( ps2 != SKILL_NONE ) 
                    sprintf( buf+strlen(buf), " & %s [%d]", skill_table[ps2].name,  skill_table[i].pre_level[1] );
                sprintf( buf+strlen(buf), "\n\r" );
                page( ch, buf );
            ...
            if( !found ) 
                send( ch, "The %s class has no %s skills.\n\r", clss_table[clss].name, skill_cat_name[j] ); 
            return;
        }
    }
...
{ 
    ...
    for( i = 0; ; i++ ) {
        ...
        if( ( trainer = mob( ch->array->list[i] ) ) != NULL && ( pTrainer = trainer->pTrainer ) != NULL && trainer->Seen( ch ) )
            break;
    }
    ...
    *buf = '\0';
    ...
    if( *argument == '\0' ) {
        for( i = 0; i < MAX_SKILL; i++ ) {
            ...
            sprintf( buf+strlen( buf ), "%-15s%7s", skill_table[i].name, arg );

            if( ch->shdata->skill[i] == 10 ) 
                strcat( buf, "\n\r" ); 
            else
                sprintf( buf+strlen( buf ), "%12d%10d\n\r", ch->shdata->level*pracs*pracs, pracs );
        }
        if( *buf == '\0' ) 
    ...
    for( i = 0; i < MAX_SKILL; i++ ) {
        ...
        ch->shdata->skill[i]++;
        send( ch, "%s teaches you %s%s.\n\r",  trainer, skill_table[i].name, ch->shdata->skill[i] == 0 ? "" : ", improving your ability at it" );
...
bool can_study( char_data* ch, int i )
{
    ...
    if( ( ps1 != NULL && ch->shdata->skill[n1] < skill->pre_level[0] ) || ( ps2 != NULL && ch->shdata->skill[n2] < skill->pre_level[1] ) ) {
...
int total_pracs( char_data* ch )
{
    ...
    for( i = 0; i < MAX_SKILL; i++ ) 
        if( ( skill = ch->shdata->skill[i] ) != 0  && ( cost = skill_table[i].prac_cost[clss] ) != -1 ) 
            total += cost*skill;
...
void char_data :: improve_skill( int i )
{
    ...
    if( species != NULL || pcdata->prac_timer > 0 || i < 0 || i > MAX_SKILL || shdata->skill[i] == 0 )
        return;
    ...
    if( number_range( 0, 30+j ) < Intelligence( ) && number_range( 0, 30+j ) < Wisdom( ) && number_range( 0, 200 ) > shdata->level ) {
        if( shdata->skill[i] == 10 || number_range( 0, 2 ) != 0 ) {
            pcdata->practice++;
            sprintf( buf, "*** You gain a practice point from %s ***\n\r\n\r", skill_table[i].name );
            send_color( this, COLOR_PRAC , buf );
        }
        else {
            shdata->skill[i]++;
            sprintf( buf, "*** You improve at %s. ***\n\r\n\r", skill_table[i].name );
            send_color( this, COLOR_PRAC , buf );
        }
    }
