...
account_data* find_account( char* name, bool all )
{
    ...
    if( !all ) 
        for( ; pos < max_account  && account_list[pos]->last_login == -1; pos++ );
            return( pos != max_account && !strncasecmp( name, account_list[pos]->name, strlen( name ) )  ? account_list[pos] : NULL );
}
...
account_data* existing_email( const char* argument )
{
    ...
    for( i = 0; i < max_account; i++ )
        if( !strcasecmp( account_list[i]->email, argument )  || !strcasecmp( account_list[i]->new_email, argument ) )
...
bool valid_email( char* addr )
{
    ...
    for( i = 0; i < strlen( addr ); i++ ) 
        if( !isalnum( addr[i] ) && addr[i] != (char) '@' && addr[i] != (char) '.' && addr[i] != (char) '_' && addr[i] != (char) '-' && addr[i] != (char) '!' && addr[i] != (char) '+' ) 
            return FALSE;
...
{  
    ...
    if( change ) {
        fprintf( fp, "Temporarily Disabled.\n" );
/*
        fprintf( fp, "A request was entered at den.mudservices.com to change\n" );
...
*/
        ...
        fprintf( fp, "A request was entered at den.mudservices.com to open an\n" );
    ...
    sprintf( tmp, "/usr/lib/sendmail -fnobody@den.mudservices.com -t %s < email.msg", account->new_email );
    system( tmp );
...
void nanny_acnt_email( link_data* link, char* argument )
{
    ...
    send( link, "   Account: %s\n\r  Password: %s\n\r     Email: %s\n\r\n\r",  account->name, account->pwd, account->new_email );
...
void nanny_acnt_request( link_data* link, char* argument )
{
    ...
    sprintf( tmp, "/usr/lib/sendmail -fnobody@den.mudservices.com -t %s < email.msg", account->email );
    system( tmp );
...
void display_pfile( char_data* ch, pfile_data* pfile, bool& first )
{
    ...
    if( first ) {
        first = FALSE;
        sprintf( tmp, "%-14s%-30s%-18s%-10s%s\n\r", "Character", "Site", "Last On", "Class", "Level" );
    ...
    sprintf( tmp, "%-14s%-30s%-18s%-10s%5d\n\r", pfile->name, host, ltime( pfile->last_on )+4, clss_table[pfile->clss].name, pfile->level );
    page( ch, tmp );
}
...
void do_accounts( char_data* ch, char* argument )
{
    ...
    if( account->banned != -1 ) 
        page( ch, "   Banned: %s\n\r", account->banned == 0 ? "forever" : ltime( account->banned ) ); 
    page( ch, "\n\r" );

    for( i = 0; i < max_pfile; i++ ) 
        if( pfile_list[i]->account == account && imm->See_Account( pfile_list[i] ) ) 
            display_pfile( ch, pfile_list[i], first );
