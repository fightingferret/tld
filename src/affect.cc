...
const char* affect_location[] = { "None", "Strength", "Dexterity",
  ...
  "Unused",  "Armor", "Hitroll", "Damroll", "Mana.regen", "Hit.regen",
  "Move.regen", "Acid", "Poison" };
...
void do_affects( char_data* ch, char* argument )
{
    ...
    if( get_trust( ch ) >= LEVEL_IMMORTAL ) {
        ...
        if( is_set( &flags, 0 ) ) {
            ...
            for( int i = 0; i < ch->affected; i++ ) {
                affect = ch->affected[i];
                page( ch, "%3d%8d    %s\n\r", affect->duration, affect->level, affect->type == AFF_NONE ? "Unknown?" : aff_char_table[ affect->type ].name );
            }
            return;
        }
    }   
    ...
    for( int i = 0; i < MAX_ENTRY_AFF_CHAR; i++ ) 
        if( is_set( ch->affected_by, i ) && *aff_char_table[i].score_name != '\0' ) {
            if( !found ) {
                found = TRUE;
...
void rem_nonself_aff( char_data* ch ) {
    ...
    for( int i = 0; i < ch->affected ; i++ ) {
        ...
        if( !is_set( plyr_race_table[ch->shdata->race].affect, i ) ) {
            for( int j = 0; j < ch->wearing; j++ )
                if( ( obj = object( ch->wearing[j] ) ) != NULL && is_set( obj->pIndexData->affect_flags, i ) )
                    objaff = TRUE;
            if ( !objaff ) {
                if( affect->leech == NULL ) 
...
void modify_affect( char_data* ch, affect_data* paf, bool fAdd )
{
    ...
    if( paf->type != AFF_NONE ) {
        ...
        paf->modifier = evaluate(aff_char_table[ paf->type ].modifier, error, 0, 0 );
    ...
    if( ch->link != NULL && ch->link->connected == CON_PLAYING && affect_name[i][0] != '\0' ) 
        send( ch, "[You feel %s %s.]\n\r", modify_amount[ min( max( mod, -mod ), 5 )-1], affect_name[i] );  
...
void add_affect( obj_data* obj, affect_data* paf )
{
    ...
    if( ( ch = character( obj->array->where ) ) != NULL ) 
        act( ch, aff_obj_table[ paf->type ].msg_on, NULL, obj );
