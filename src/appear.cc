...
// BEGIN CHANGES BY SADIS
int display( char_data* ch, request_array& array, const char* word,
  bool& first )
{
  int  req_count=0;
  ...
  {
    if( ( ch->pcdata->trust > LEVEL_AVATAR ) && ( word == "Immortal" ) )
      {
      page( ch, "%d -- %-15s %s\n\r", request_app.size+i+1, array[i]->pfile->name,
        array[i]->reason );
      req_count=req_count+1;
      }
    else if( ( ch->pcdata->trust <= LEVEL_AVATAR ) && ( word == "Immortal" ) )
      {
      page( ch, "%-15s \n\r", array[i]->pfile->name );
      req_count=req_count+1;
      }
    else if( ( ch->pcdata->trust >= LEVEL_AVATAR ) && ( word == "Avatar" ) )
      {
      page( ch, "%d -- %-15s %s\n\r", i+1, array[i]->pfile->name,
        array[i]->reason );
      req_count=req_count+1;
      }
    else if( ( ch->pcdata->trust < LEVEL_AVATAR && word == "Avatar" ) )
...
void remove_req( request_array& array, int element )
{
...
void do_request( char_data* ch, char* argument )
{
  ...
  if( *argument == '\0' ) {
    bool first = TRUE;
    req_count_av=display( ch, request_app, "Avatar", first );
    req_count_imm=display( ch, request_imm, "Immortal", first );

    req_count_total = req_count_av + req_count_imm;

    if( first )
      send( ch, "Request queue is empty.\n\r" );
    else {
      send( ch, "%d requests for an avatar are awaiting response.\n\r", req_count_av );
      send( ch, "%d requests for an immortal are awaiting response.\n\r", req_count_imm ); }
    return;
    }
 
  if( number_arg( argument, req_num_remove ) && ch->pcdata->trust >= LEVEL_AVATAR ) {
    is_req_imm = is_req_av = TRUE;

    // need to know the number of elements in these arrays
    req_count_imm = request_imm.size;
    req_count_av = request_app.size;

    // need to know if there is an element0 in these here arrays
    if( request_imm.size == 0 )
      is_req_imm = FALSE;
    if( request_app.size == 0 )
      is_req_av = FALSE;

    req_count_total = req_count_av + req_count_imm;

    if( req_num_remove > req_count_total || req_count_total == 0 ) {
      send( ch, "Try again butthead, that request doesn't exist.\n\r" );
      return;
      }
    if( req_num_remove <= 0 ) {
      send( ch, "Yeah, I might let you do that.\n\r" );
      return;
      }
    if( req_num_remove <= req_count_av ) {
      remove_req( request_app, req_num_remove-1 );
      send( ch, "Request removed.\n\r" );
      return;
      }
    if( ch->pcdata->trust > LEVEL_AVATAR ) {
      remove_req( request_imm, req_num_remove - req_count_av - 1 );
      send( ch, "Request removed.\n\r" );
      return;
      }
    send( ch, "Unable to remove request.\n\r" );
    return;
/*
    clear_screen( ch );
    reset_screen( ch );
