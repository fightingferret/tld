...
void do_roomlist( char_data* ch, char* )
{
  ...
  page_underlined( ch, "Vnum     Name of Room                   Player     Linked    Typo  Acode\n\r" );

  for( room = room->area->room_first; room != NULL; room = room->next ) {
    for( int i = 0; i < room->contents; i++ ) {
      if( ( rch = player( room->contents[i] ) ) != NULL )
        if( ( imm = wizard( rch ) ) == NULL ) {
          mortal = TRUE;
          break;
          }
    }
    strncpy( tmp, room->name, 30 );
    tmp[30] = '\0';
    page( ch, "%-6d   %30s  %-6s  %-10s  %-4s  %s\n\r", room->vnum, 
        tmp, mortal ? "Mortal" : "", 
        ( room->exits == NULL ) ? "Not Linked" : "",
        ( strlen( room->comments ) > 0 ) ? "Typo" : "",
        ( room->action == NULL ) ? "" : "Acode" );
    mortal = FALSE;
  }
}
