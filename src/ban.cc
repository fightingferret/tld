...
tban_data* tban_list = NULL;

tban_data :: tban_data( )
{
  name = empty_string;

  record_new( sizeof( tban_data ), MEM_BAN );
  append( tban_list, this );
};
...
tban_data :: ~tban_data( )
{
  ...
  record_delete( sizeof( tban_data ), MEM_BAN );
  remove( tban_list, this );
};
// Sadis -- Done declaring tban class
...
// SADIS -- add flag for total site bans -t-
  if( !get_flags( ch, argument, &flags, "srnt", "Ban" ) )
    return;
...
//SADIS -- TOTAL SITE BANS
  if( is_set( &flags, 3 ) ) {
    if( *argument == '\0' ) {
      page_title( ch, "Banned Sites" );
      for( tban = tban_list; tban != NULL; tban = tban->next )
        page( ch, "%s\n\r", tban->name );
      return;
      }
    ...
    if( !is_set( &flags, 1 ) ) {
      for( tban = tban_list; tban != NULL; tban = tban->next ) {
        if( !strcasecmp( argument, tban->name ) ) {
          send( "That site is already banned!\n\r", ch );
          return;
          }
        }
      tban        = new tban_data;
      tban->name  = alloc_string( argument, MEM_BAN );
      save_tsbanned( );
      send( ch, "Ok.\n\r" );
      }
    else {
...
// SADIS -- ALL DONE WITH TOTAL SITE BAN
 ...
 *   IS_BANNED ROUTINE
 */
// Sadis -- total ban checking function
bool is_tbanned( const char* host )
{
  tban_data*     tban;
  ...
  for( tban = tban_list; tban != NULL; tban = tban->next ) {
    ban_len = strlen( tban->name );
    if( ban_len <= host_len
      && !strcasecmp( &host[host_len-ban_len], tban->name ) )
      break;
    }
...
// SADIS -- TOTAL SITE BAN LOAD/READ
void load_tsbanned( void )
{
  tban_data*    ban;
  char*       name;
  FILE*         fp;

  fprintf( stderr, "Loading Totally Banned Sites...\n\r" );

  if( ( fp = fopen( TS_BANNED_FILE, "r" ) ) == NULL ) {
    perror( TS_BANNED_FILE );
    exit( 1 );
    }

  if( strcmp( fread_word( fp ), "#BANNED" ) )
    panic( "Load_tsbanned: missing header" );
...
void save_tsbanned( )
{
  ...
  fp = open_file( TS_BANNED_FILE, "w" );

  fprintf( fp, "#BANNED\n\n" );

  for( ban = tban_list; ban != NULL; ban = ban->next )
    fprintf( fp, "%s~\n", ban->name );
  fprintf( fp, "$~\n" );
...
//SADIS -- DONE TOTAL BAN LOAD/SAVE
