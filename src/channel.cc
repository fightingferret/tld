...
flag_data lang_flags = { "Languages", &skill_table[LANG_PRIMAL].name, &skill_table[LANG_PRIMAL+1].name, &max_language };
...
void         display        ( tell_data*&, char_data*, char*, char_data* = NULL );
...
void display( tell_data*& list, char_data* ch, char* text, char_data* victim )
{
...
void do_language( char_data *ch, char *argument )
{
    ...
    if( ch->pcdata->speaking < LANG_PRIMAL || ch->pcdata->speaking >= MAX_LANGUAGE )
    ...
    for( i = LANG_PRIMAL; i < MAX_LANGUAGE; i++ )
        if( fmatches( argument, skill_table[i].name ) ) {
            ...
            if( ch->shdata->skill[i] == UNLEARNT && !is_set( ch->affected_by, AFF_TONGUES ) ) {
...
void do_ooc( char_data* ch, char* argument )
{
    ...
    for( int i = 0; i < player_list; i++ ) {
        ...
        if( hear_channels( victim ) && is_set( victim->pcdata->pfile->flags, PLR_OOC ) && !victim->Filtering( ch ) ) 
...
void do_chant( char_data* ch, char* argument )
{
    ...
    for( int i = 0; i < player_list; i++ ) {
        victim = player_list[i];
        if( hear_channels( victim ) && victim->pcdata->religion == ch->pcdata->religion ) {
            add_tell( victim->chant, ch->Seen_Name( pc ), tmp, language ); 
            if( ( is_set( pc->pcdata->pfile->flags, PLR_CHANT ) ) && ( victim != pc ) ) {
...
void do_chat( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Chat channel", ch->pcdata->pfile->flags, PLR_CHAT ) )
        return;
    ...
    for( int i = 0; i < player_list; i++ ) {
        ...
        if( hear_channels( victim ) && are_allied( ch, victim ) ) { 
            ...
            if( victim != ch && victim->link != NULL && is_set( victim->pcdata->pfile->flags, PLR_CHAT ) && !victim->Filtering( ch ) ) {
                sprintf( buf, "%s chats%s:", who_name( victim, ch ), slang( victim, language ) );
                *buf = toupper( *buf ); 
                length = strlen( buf );
                sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &tmp[length < max_length] );
...
void do_gossip( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Gossip channel", ch->pcdata->pfile->flags, PLR_GOSSIP ) ) 
        return;
    ...
    for( int i = 0; i < player_list; i++ ) {
        ...
        if( hear_channels( victim ) ) {
            ...
            if( victim != ch && victim->link != NULL && is_set( victim->pcdata->pfile->flags, PLR_GOSSIP ) && !victim->Filtering( ch ) ) {
...
void do_atalk( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Auction channel", ch->pcdata->pfile->flags, PLR_ATALK ) )
        return;
    ...
    for( int i = 0; i < player_list; i++ ) {
        victim = player_list[i];
        if( hear_channels( victim ) && is_set( victim->pcdata->pfile->flags, PLR_ATALK ) && !victim->Filtering( ch ) ) 
...
void do_avatar( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Avatar channel", ch->pcdata->pfile->flags, PLR_AVATAR ) )
        return;
    ...
    *buf   = toupper( *buf ); 
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i];
        if( hear_channels( pc ) && ( imm = wizard( pc ) ) != NULL && has_permission( imm, PERM_AVATAR_CHAN ) ) {
            add_tell( imm->avatar, ch->real_name( ), tmp ); 
            if( is_set( imm->pcdata->pfile->flags, PLR_AVATAR ) && !imm->Filtering( ch ) ) {
                sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &tmp[length < max_length] );
                send_color( imm, COLOR_IMM_CHANN, buf );
            }
        }
    }
...
void do_buildchan( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Creator channel", ch->pcdata->pfile->flags, PLR_BUILDCHAN ) )
        return;
    ...
    sprintf( buf, "[BUILD] %s:", ch->descr->name );
    *buf   = toupper( *buf ); 
    length = strlen( buf );
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i];
        if( hear_channels( pc ) && ( imm = wizard( pc ) ) != NULL && has_permission( imm, PERM_BUILD_CHAN ) ) {
...
void do_immtalk( char_data* ch, char* argument )
{
    ...
    *buf   = toupper( *buf ); 
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i];
        if( hear_channels( pc ) && ( imm = wizard( pc ) ) != NULL && has_permission( imm, PERM_IMM_CHAN ) ) {
...
void do_god( char_data* ch, char* argument )
{
    ...
    *buf   = toupper( *buf ); 
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i];
        if( hear_channels( pc ) && ( imm = wizard( pc ) ) != NULL && has_permission( imm, PERM_GOD_CHAN ) ) {
...
void do_say( char_data* ch, char* argument )
{
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        ...
        sprintf( buf, "%s %s%s:", ch->Name( victim ), say_verb( ch, victim ), slang( victim, language ) );
        *buf = toupper( *buf );
        length = strlen( buf );
        garble_string( arg, tmp, get_language( victim, language ) );
        sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &arg[length < max_length] );
...
void process_tell( char_data* ch, char_data* victim, char* argument )
{
    ...
    if( ch->pcdata != NULL && victim->pcdata != NULL ) {
        if( victim->Filtering( ch ) ) {
            fsend( ch, "%s is filtering you - please leave %s in peace.", victim->descr->name, victim->Him_Her( ) );
            return; 
        }
    ...
    if( ( pc = player( victim ) ) == NULL && victim->link != NULL )
    ...
    if( victim->pcdata != NULL ) {
        if( victim->link == NULL && pc->switched == NULL ) 
            ...
            tell_message( ch, "%s has been idle for %d seconds", victim, current_time-victim->timer );
    ...
    sprintf( buf, "%s tells %s:", ch->Seen_Name( victim ), victim != pc || pc->switched == NULL ? "you" : pc->descr->name );
    *buf = toupper( *buf );
    length = strlen( buf );
    sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &tmp[length < max_length] );
...
// This currently doesn't work
    if( is_set( &victim->status, STAT_AFK ) ) {
        if( victim->pcdata->afk_message != "\0" ) {
            process_tell( victim, ch, victim->pcdata->afk_message );
        }
        else {
            process_tell( victim, ch, "Your message was recieved, however I am currently AFK." );
        }
    }
//    
...
void do_tell( char_data *ch, char *argument )
{
    ...
    if( ( victim = one_character( ch, arg, "tell", (thing_array*) &player_list ) ) == NULL )
        return;
...
void trigger_say( char_data* ch, char_data* mob, char* argument )
{
    ...
    for( mprog = mob->species->mprog; mprog != NULL; mprog = mprog->next ) 
        if( mprog->trigger == MPROG_TRIGGER_TELL && ( is_name( argument, mprog->string ) || *mprog->string == '\0' ) ) {
            var_ch   = ch;
            var_mob  = mob;
            var_arg  = argument;
...
{ 
    ...
    int            length;
    int        max_length;
    char          afk_buf [ MAX_STRING_LENGTH ];
    ...
    sprintf( buf, "%s %ss to you%s:", ch->Name( victim ), verb, slang( victim, language ) );
    buf[0] = toupper( buf[0] );
    length = strlen( buf );
    garble_string( arg, tmp, get_language( victim, language ) );
    sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &arg[length < max_length] );
...
// IC AUTOMATED REPLY FOR AFKNESS
    if( is_set( &victim->status, STAT_AFK ) ) {
        if( victim->pcdata->afk_message != "\0" ) {
            sprintf( afk_buf, "%s %s", ch->descr->name, victim->pcdata->afk_message ); 
            ic_tell( victim, afk_buf, verb );
        }
        else {
            sprintf( afk_buf, "%s Your message was recieved, however I am currently AFK." );
            ic_tell( victim, afk_buf, verb);
        }
    }
// 
...
void do_emote( char_data *ch, char *argument )
{
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( rch = character( ch->array->list[i] ) ) == NULL || rch->position <= POS_SLEEPING )
            continue; 
...
void do_gtell( char_data* ch, char* argument )
{
    ...
    sprintf( buf, "You tell your group%s:", slang( ch, language ) );
    length = strlen( buf );
    sprintf( &buf[length], "%s%s", length < max_length ? "" : "\n\r", &tmp[length < max_length] );
    send( buf, ch );
...
void do_ctell( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Clan channel", ch->pcdata->pfile->flags, PLR_CTELL ) )
        return;
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i];
        if( pc->In_Game( ) && pc->pcdata->pfile->clan == ch->pcdata->pfile->clan ) {
            add_tell( pc->ctell, ch->Seen_Name( pc ), tmp ); 
            if( pc != ch && pc->link != NULL && is_set( pc->pcdata->pfile->flags, PLR_CTELL ) ) {
...
void do_review( char_data* ch, char* argument )
{
    ...
    if( matches( argument, "ctell" ) ) {
        display( victim->ctell, ch, "ctells", victim );
        return;
    }
