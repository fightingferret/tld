...
bool is_incognito( pfile_data* pfile, char_data* ch )
{
    ...
    return( pfile->clan != ch->pcdata->pfile->clan || pfile->clan == NULL );
}
...
char* adjectives( char_data* ch, char_data* victim )
{
    ...
    *tmp = '\0';
    ...
    if( is_set( victim->affected_by, AFF_FIRE_SHIELD ) && ( victim->species == NULL || !is_set( victim->species->affected_by, AFF_FIRE_SHIELD ) ) )
        sprintf( tmp+strlen( tmp ), "%sfire-shrouded", *tmp == '\0' ? "" : ", " ); 

    if( is_set( victim->affected_by, AFF_DISPLACE ) && ( victim->species == NULL || !is_set( victim->species->affected_by, AFF_DISPLACE ) ) )
...
const char* char_data :: Seen_Name( char_data* ch, int num, bool )
{
    ...
    if( species == NULL ) {
        if( ch == NULL || !in_character || ch->Recognizes( this ) ) 
            return descr->name;
        sprintf( tmp, "a%s %s", isvowel( *descr->singular ) ? "n" : "", descr->singular );
        return tmp;
    }
    ...
    if( num > 1 ) {
        sprintf( tmp, "%s %s", number_word( num ),
...
const char* char_data :: Long_Name( char_data* ch, int num )
{
    ...
    if( num > 1 ) {
        ...
        sprintf( tmp, "%s%s%s %s%s%s%s%s%s", prefix_p, prefix_p == empty_string ? "" : " ", number_word( num ), adj, adj_p, adj_p == empty_string ? "" : " ",
            color_code( ch, species->color ), plural, normal( ch ) );
        return tmp;
    }
    ...
    if( species == NULL ) {
        if( ch->Recognizes( this ) ) {
            if( !is_set( &ch->pcdata->message, MSG_LONG_NAMES ) )
                return descr->name;
            sprintf( tmp, "%s, %s %s%s,", descr->name, article, adj, singular );
        }
        else {
            sprintf( tmp, "%s %s%s", article, adj, singular );
        }
        return tmp;
    }
    ...
    if( ( known && descr->name != empty_string ) || ( pet_name != empty_string && leader == ch ) ) {
        if( *singular != '\0' && is_set( &ch->pcdata->message, MSG_LONG_NAMES ) )
            ...
            sprintf( tmp+strlen(tmp), "%s%s%s", color_code( ch, species->color ), pet_name != empty_string ? pet_name : descr->name, normal( ch ) );
    }
    else {
...
// Sadis.. addresses multiple instances showing up as 'is' instead of 'are'
                    sprintf( tmp, "%s %s here, fighting %s%s",
                        Long_Name( ch, num ),
                        num == 1 ? "is" : "are",
                        fighting == ch ? "YOU!" : fighting->Name( ch ), fighting == ch ? "" : "." );
