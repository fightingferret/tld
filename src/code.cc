...
void* proc_arg( struct arg_type* arg )  
{
  ...
  if( arg->family == loop ) {
    ...
    for( i = 0; i < var_room->contents; i++ ) {
      ...
      proc_arg( aloop->aloop );
      if( var_room == NULL ) {
        if( last_var_room == NULL ) {
          break; }
        else {
          var_room = last_var_room;
	  i= i - 1 ;
          continue;
	  }
        }
      last_var_room = var_room;
// SADIS - This part here deals with when a mob gets moved from inside a
// loop
      if( var_rch != rch || rch->in_room != var_room ) {
        i--;
        continue;
      }
    }
