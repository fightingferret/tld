...
const char* color_fields [ MAX_COLOR ] = { "default", "room_name", "tell",
    "say", "gossip", "players", "mobs", "objects", "unused", "unused",
    "titles", "ctell", "chat", "ooc", "gtell", "auction", "info", 
    "to_self", "to_group", "by_self", "by_group",  
    "mild_emphasis", "strong_emphasis", "black", "red1", "red2", "green1",
    "green2", "yellow1", "yellow2", "blue1", "blue2",
    "magenta1", "magenta2", "cyan1", "cyan2", "white1", "white2", "imm_channels", "arena",
    "practice", "bash_to", "bash_from", "bash_other" };

const char* format_vt100 [] = { "none", "bold", "reverse", "underline" };
...
const char* ansi_codes( int code )
{
    ...
    sprintf( tmp+15*pntr, "[%d;%d;%d;%dm", array[3],array[2],array[1],array[0] );
...
void do_color( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        ...
        for( i = 0; i < MAX_TERM; i++ ) {
            page( ch, "%18s (%1c)%s", term_table[i].name, term == i ? '*' : ' ', i%3 == 2 ? "\n\r" : "" );
      }
    if( i%3 != 0 )
        page( ch, "\n\r" );
    page( ch, "\n\r" );
    ...
    for( i = 0; i < MAX_COLOR; i++ ) {
        page( ch, "%16s : %s%s%s\n\r", color_fields[i], color_code( ch, i ),
        term == TERM_VT100 ? term_table[ TERM_VT100 ].format[ch->pcdata->color[i] ] : ansi_colors( ch->pcdata->color[i] ),
            normal( ch ) );
    }
    ...
    if( term == TERM_ANSI ) {
        ...
        send( ch, "Color for %s set to %s.\n\r", color_fields[i], term == TERM_VT100
    ...
    send( ch, "Format of %s set to %s.\n\r", color_fields[i], term_table[term].format[j] );
...
void send_color( char_data* ch, int type, const char* msg )
{
    ...
    if( term != TERM_DUMB ) 
        send( ch, "%s%s%s", term_table[term].codes( color ), msg,
...
void page_color( char_data* ch, int type, const char* msg )
{
    ...
    if( term != TERM_DUMB ) 
        page( ch, "%s%s%s", term_table[term].codes( color ), msg,
            term_table[term].codes( ch->pcdata->color[ COLOR_DEFAULT ] ) );
    else 
        page( ch, msg );
