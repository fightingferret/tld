...
void          msg_attack      ( char_data*, char_data*, const char*, const char*, char );
void          msg_other       ( char_data*, char_data*, const char*, const char*, char );
...
void dam_local( char_data* victim, char_data* ch, int damage, const char* dt, const char* loc_name )
{
    char               tmp  [ TWO_LINES ];
    char             punct;
    const char*   dam_name;
    ...
    sprintf( tmp, "%s's %s %s%s%s your %s%c\n\r", ch->Name( victim,1, TRUE ),
        dt, to_self( victim ), dam_name, normal( victim ), loc_name, punct );
    corrupt( tmp, TWO_LINES, "Dam_Local" );
    *tmp = toupper( *tmp );
    send( victim, tmp );
 
    if( victim->Seen( ch ) ) 
        sprintf( tmp, "Your %s %s%s%s %s's %s%c", dt, by_self( ch ), dam_name, normal( ch ),
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( rch = character( ch->array->list[i] ) ) == NULL
            ...
            continue;
        if( victim->Seen( rch ) ) 
            sprintf( tmp, "%s's %s %s%s%s %s's %s%c", ch->Name( rch, 1, TRUE ), dt, damage_color( rch, ch, victim ),
                dam_name, normal( rch ), victim->Seen_Name( rch ), loc_name, punct );
        else
            sprintf( tmp, "%s's %s %s%s%s someone%c", ch->Name( rch ), dt, damage_color( rch, ch, victim ),
...
void msg_attack( char_data* victim, char_data* ch, const char* dt, const char* word, char punct )
{
    ...
    sprintf( tmp, "%s's %s %s%s%s you%c\n\r", ch->Name( victim ), dt, to_self( victim ), word, normal( victim ), punct );
    *tmp = toupper( *tmp );
    send( tmp, victim );
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( rch = character( ch->array->list[i] ) ) == NULL
            ...
            || !is_set( &rch->pcdata->message, flag ) || !rch->Can_See( ) )
            continue;
        sprintf( tmp, "%s's %s %s%s%s %s%c", ch->Name( rch ), dt,
            damage_color( rch, ch, victim ), word, normal( rch ), victim->Name( rch ), punct );
        *tmp = toupper( *tmp );
        strcat_cond( tmp, rch, victim );
    }
...
void msg_other( char_data* victim, char_data* ch, const char* dt, const char* word, char punct )
{
    ...
    sprintf( tmp, "%s %s%s%s you%c\n\r", dt, to_self( victim ), word, normal( victim ), punct );
    ...
    *tmp = toupper( *tmp );
    ...
    for( int i = 0; i < *victim->array; i++ ) {
        if( ( rch = character( victim->array->list[i] ) ) == NULL || rch == victim || !rch->Can_See( ) )
            continue;
        sprintf( tmp, "%s %s%s%s %s%c", dt, damage_color( rch, ch, victim ), word, normal( rch ),
...
bool elemental_inflict( char_data* victim, char_data* ch, int dam, char* dt, int type )
{
    ...
    switch( type ) {
        case 1:
            dam *= 100-victim->Save_Fire( );
            dam /= 100;
            break;
        case 2:
            dam *= 100-victim->Save_Cold( );
            dam /= 100;
            break;
        case 3:
            dam *= 100-victim->Save_Acid( );
            dam /= 100;
            break;
        case 4:
            dam *= 100-victim->Save_Shock( );
            dam /= 100;
            break;
    }
    ...
    if( ch != NULL && ch->species != NULL && !is_set( &ch->status, STAT_PET ) )
...
bool inflict( char_data* victim, char_data* ch, int dam, char* dt )
{
    ...
    if( ch != NULL && ch->species != NULL && !is_set( &ch->status, STAT_PET ) ) 
        ch->species->damage += dam;
    if( !victim->Seen( ch ) && ch->species == NULL )
...
void glance( char_data* ch, char_data* victim )
{
    ...
    page( ch, "%s %s %s%s%s%c\n\r", victim->Seen_Name( ch ), condition_prep( victim ),
        color_scale( ch, 6-6*victim->hit/victim->max_hit ), condition_word( victim ),
...
void spam_room( const char* text, char_data* ch, char_data* victim )
{
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( rch = character( ch->array->list[i] ) ) == NULL
            || rch->link == NULL || rch == ch || rch == victim || rch->position <= POS_SLEEPING
...
{
    dam_message( victim, ch, damage, string, lookup( physical_index, damage, plural ) );
