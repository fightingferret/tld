...
void death( char_data* victim, char_data* ch, char* dt )
{
    ...
    if (victim->species == NULL)
        if( ( room = Room( victim->array->where ) ) != NULL && is_set( &room->room_flags, RFLAG_ARENA ) ) {
            sprintf( atmp, "%s is killed by %s in arena combat.", victim->Name( ), ch->Name( ) );
            info( atmp, LEVEL_BUILDER, atmp, IFLAG_ARENA, 1, victim );
        }  
...
void register_death( char_data* victim, char_data* ch, char* dt )
{
    ...
    if( victim->species != NULL ) {
        if( is_set( &victim->status, STAT_PET ) && victim->leader != NULL && victim->leader->pcdata != NULL ) {
    ...
    sprintf( tmp1, "Killed by %s at %s.", ch == NULL ? dt : ch->real_name( ), victim->array->where->Location( ) );
...
void death_message( char_data* victim )
{
    ...
    for( int i = 0; i < *victim->array; i++ ) {
        ...
        if( victim->species != NULL && is_set( &victim->species->act_flags, ACT_MELT ) ) {
            sprintf( tmp1, "  +-- %%%ds --+\n\r", strlen( name )+11 );
            sprintf( tmp2, tmp1, "" );
...
const char* fragments_msg = "The fragments from %s quickly melt, leaving %s %s.";   
...
void loot_corpse( obj_data* corpse, char_data* ch, char_data* victim )
{
    ...
    thing_array * array = new thing_array;

    if( level == 2 ) {
        for( int a = 0; a < corpse->contents.size; ++a )
            if( ( obj = object( corpse->contents[a] ) ) != NULL )
    ...
    for (int a = 0; a < corpse->contents.size; ++ a){
        ...
        *array += object (corpse->contents [a]);
    }
    get_obj (ch, array, corpse);
    if( is_set( ch->pcdata->pfile->flags, PLR_AUTO_SPLIT ) ) 
        split_money( ch, amount, FALSE );
