...
void do_open( char_data* ch, char* argument )
{
  ...
  if( ( thing = one_thing( ch, argument, "open", (thing_array*) &ch->in_room->exits, ch->array, &ch->contents, &ch->wearing ) ) == NULL )
    return;
...
void do_close( char_data* ch, char* argument )
{
  ...
  if( ( thing = one_thing( ch, argument, "close", (thing_array*) &ch->in_room->exits, ch->array, &ch->contents, &ch->wearing ) ) == NULL )
    return;
...
void do_unlock( char_data* ch, char* argument )
{
    ...
    if( ( thing = one_thing( ch, argument, "unlock", (thing_array*) &ch->in_room->exits, ch->array, &ch->contents ) ) == NULL )
        return;
...
void do_pick( char_data* ch, char* argument )
{
  ...
  exit_data*     door; 
    ...
    if( ch->fighting != NULL ) {
    	send( ch, "Turning your back on a fight is dangerous.\n\r" );
    	return;
  }
  if( ( thing = one_thing( ch, argument, "pick",
   ...
   if ( ( obj = find_type( ch->contents, ITEM_LOCK_PICK ) ) == NULL) {
		send( ch, "You need a lock pick to pick locks.\n\r");
		return;
	}
