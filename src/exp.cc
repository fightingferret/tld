...
void gain_exp( char_data* ch, char_data* victim, int gain, const char* msg )
{
  ...
  int total_al;
  int total_nat;
// This here is for that -'ve percentages..
  for( int i = 0; i < MAX_ENTRY_ALIGNMENT; i++ )
   total_al += pc->reputation.alignment[i];
  if( (total_al+1) < 0 )
    for( int i = 0; i < MAX_ENTRY_ALIGNMENT; i++ )
      pc->reputation.alignment[i] /= 2;
...
// Sadis -- deaths of pets shouldn't count toward species totals
