...
void react_attack( char_data* ch, char_data* victim )
{
  ...
  init_attack( victim, ch );
    ...
    if( (victim->species == NULL) && (victim->fighting != ch) )
        if( ( victim->in_room != NULL ) && ( is_set( &victim->in_room->room_flags, RFLAG_ARENA) ) ) {
            sprintf( tmp, "%s attacked by %s in the arena.", victim->Name( ), ch->Name( ) );
            info( tmp, LEVEL_BUILDER, tmp, IFLAG_ARENA, 2, victim );
        }
