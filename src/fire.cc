...
bool fireball_effect( char_data *ch, char_data *victim, int level )
{
  ...
  if( do_blast
      && i < 6 ) {
    exit = exit_direction( victim->in_room, i );

    if( exit == NULL || !exit->Seen( victim ) )
      do_blast = FALSE;

    if( do_blast
        && is_set( &exit->exit_info, EX_CLOSED )
        && !is_set( victim->affected_by, AFF_PASS_DOOR ) )
          do_blast = FALSE;

    if( do_blast
        && is_set( &exit->exit_info, EX_REQUIRES_CLIMB )
        && !can_climb( victim ) && !victim->can_fly( ) ) 
          do_blast = FALSE;
      }
    else
      do_blast = FALSE;
