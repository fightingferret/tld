...
void do_flee( char_data* ch, char* argument )
{
  ...
  if( ch->position < POS_FIGHTING ) {
    send( ch, "It might be a good idea to stand up before trying to flee!\n\r" );
    return;
    }
...
bool attempt_flee( char_data* ch, exit_data* exit )
{
  ...
  if( ch->position < POS_FIGHTING ) {
    send(ch,"You need to stand before you can flee!\n\r");
    return FALSE;
  }

  if( is_set( ch->affected_by, AFF_ENTANGLED ) )
  {
    send( ch, "You are entangled and cannot flee.\n\r" );
    return FALSE;
  }
  ...
  if( number_range( 0, 100 ) < 10+20*ch->get_burden() ) {
    ...
    if(ch->species == NULL) 
        if( ( room = Room( ch->array->where ) ) != NULL && is_set( &room->room_flags, RFLAG_ARENA ) ) {
            sprintf( tmp, "%s has fled the battle.", ch->Name( ) );
            info( tmp, LEVEL_BUILDER, tmp, IFLAG_ARENA, 2, ch );
        }
