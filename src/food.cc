...
const char*  cook_list [] = { "raw", "cooked", "burnt", "neither" };
...
void do_cook( char_data* ch, char* argument )
{
  ...
  if( ( obj = one_object( ch, argument, "cook", &ch->contents ) ) == NULL )  {
    send( "You do not have that item.\n\r", ch );
    return;
    }
 
  if ( ( fire = find_type( *ch->array, ITEM_CAMP ) ) == NULL) {
    if ( ( fire = find_type( *ch->array, ITEM_FIRE ) ) == NULL) {
    send( ch, "You see nothing in the room to cook on.\n\r");
    return;
    }
  }
