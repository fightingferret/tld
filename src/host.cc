...
// SADIS -- TOTAL SITE BAN
// relocated to prevent -any- connection of any variety from a banned
// address
  if( is_tbanned( link->host ) ) {
    help_link( link, "Sitebanned" );
    sprintf( tmp, "Connection from banned address %s", link->host );
    info( "", LEVEL_DEMIGOD, tmp, IFLAG_BAN_INFO, 2 );
    close_socket( link, TRUE );
    return;
    }
// ALL DONE WITH SITE BAN THING
