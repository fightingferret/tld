...
const char* imm_title [] = { "Avatar", "Apprentice", "Builder", "Architect", "Immortal", "Spirit", "Angel", "Demigod", "God" };

const char* permission_name [ MAX_PERMISSION+1 ] = {
    "none", "all_mobs", "all_objects", "all_rooms", "approve", "basic", "build_chan", "commands", "echo", "god_chan",
    "goto", "help files", "imm_chan", "lists", "load_objects", "misc.tables", "mobs", "noteboard", "objects", "players", "quests",
    "reimb_exp", "reimb_equip", "rooms", "shutdown", "site.names", "snoop", "spells", "socials", "transfer", "unfinished", "write.all", "write.areas",
    "avatar.Chan", "clans", "rtables", "disabled", "force.players", "adjust.gsp", "pbug" };
...
void do_gsp( char_data* ch, char* argument )
{
    ...
    if( *arg == '\0' || *argument == '\0' || !is_number( argument ) ) {
        send( ch, "Syntax: gsp <char> <amount to dock>.\n\r" );
        return;
    }

    if( ( victim = one_player( ch, arg, "gsp", (thing_array*) &player_list ) ) == NULL )
        return;
    ...
    if( victim->pcdata->trust >= ch->pcdata->trust ) {
        send( ch, "%s wouldn't appreciate you doing that.\n\r", victim );
        return;
    }

    if( ( amnt = atoi( argument ) ) < 0 || amnt > 1000 ) {
        send( ch, "Docked amount must be between 0 and 1000.\n\r" );
        return;
    }

    if( ( pc->gossip_pts - amnt ) <= -1000 ) {
        send( ch, "%s doesn't have enough to dock.\n\r", victim );
        return;
    }

    pc->gossip_pts -= amnt;
    send( ch, "%d gossip points deducted from %s", amnt, victim );
    return;
}
...
void do_reimburse( char_data* ch, char* argument )
{
    ...
    array =  ( is_set( &flags, 1 ) ? &link.player->contents : &link.player->locker );
    ...
    fsend( ch, "Finding the status of %s from earlier times you duplicate %s equipment, dropping it to the ground.", victim, victim->His_Her( ) );
    send( victim, "You feel %s probing your history ...\n\r", ch );
    send( victim, "With a sweep of %s arms %s produces your old equipment.\n\r", ch->His_Her( victim ), ch->He_She( victim ) );
    }
...
void do_goto( char_data* ch, char* argument )
{
    ...
    for( int i = from->contents-1; i >= 0; i-- ) 
        if( ( rch = character( from->contents[i] ) ) != NULL && rch->leader == ch && is_set( &rch->status, STAT_PET ) )
...
void do_at(char_data *ch, char *argument)
{
    ...
    if( *argument == '\0' ) {
        send( ch, "Usage: at <location> <command>\n\r" );
        return;
    }

    argument = one_argument( argument, arg );
    if( ( new_room = find_location( ch, arg ) ) == NULL )
        return;

    old_room = ch->in_room;

    if( is_set( &new_room->room_flags, RFLAG_OFFICE ) && !is_god( ch ) && new_room->vnum  != imm->office ) {
        send( ch, "That location belongs to another god.\n\r" );
        return;
    }
...
void transfer( char_data* ch, char_data* victim, room_data* room )
{
    ...
    for( int i = *where-1; i >= 0; i-- ) 
        if( ( rch = character( where->list[i] ) ) != NULL && rch->leader == victim && is_set( &rch->status, STAT_PET ) )
...
void do_transfer( char_data* ch, char* argument )
{
    ...
    if( !strcasecmp( arg, "all" ) ) {
        for( int i = 0; i < player_list; i++ ) {
            ...
            if( player->In_Game( ) && player->shdata->level < LEVEL_APPRENTICE && player->in_room != room )
...
// Sadis -- un fucked this function
    ...
    for( area = area_list; area != NULL; area = area->next ) {
        ...
        if( ch->in_room->vnum > max || ch->in_room->vnum < min )
            continue;
        for( room = area->room_first; room != NULL; room = room->next ) {
            for( int popup = 0 ; popup < room->contents ; popup++ ) {
                if( ( victim = player( room->contents[popup] ) ) != NULL ) {
                    found = TRUE;
                    sprintf( buf, "%-28s %s\n\r", victim->descr->name, victim->in_room->name );
                    send( ch, "%s", buf );
                }
            }
        }
...
void do_purge( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        ...
        for( int i = *ch->array-1; i >= 0; i-- )
            if( ( victim = character( ch->array->list[i] ) ) == NULL || ( victim->pcdata == NULL && !is_set( &victim->status, STAT_PET ) ) )
...
void do_restore( char_data* ch, char* argument )
{
    ...
    if( !strcasecmp( argument, "all" ) ) {
        ...
        for( int i = 0; i < player_list; i++ ) {
            ...
            if( victim->In_Game( ) && victim->shdata->level < LEVEL_APPRENTICE )
                restore( victim, ch );
        }
...
void restore( char_data* victim, char_data* ch )
{
    ...
    strip_affect( victim, AFF_CAMPING );
...
void do_sset( char_data* ch, char* argument )
{
    ...
    if( !is_number( arg3 ) ) {
        //arg3 = arg4;
        //fprint( arg2, "%s %s" arg2, arg3);
        //if( !is_number( arg4 ) ) {
            send( ch, "Syntax: sset <char> [name/(name)] [value]\n\r" );
            //return;
       // }
    }
    ...
    for( sn = 0; sn < MAX_SKILL; sn++ ) {
        ...
        if( !strncasecmp( skill_table[sn].name, arg2, strlen( arg2 ) ) ) {
            victim->shdata->skill[sn] = value;
            send( ch, "%s set on %s to %d.\n\r", skill_table[sn].name, victim->descr->name, value );
            return;
        }
    }
...
void do_holylight( char_data* ch, char* )
{
    ...
    send( ch, "Holy light mode %s.\n\r", on_off( ch->pcdata->pfile->flags, PLR_HOLYLIGHT ) );
...
void do_snoop( char_data* ch, char* argument )
{
    ...
    if( argument[0] == '\0' ) {
        ...
        for( link = link_list; link != NULL; link = link->next ) 
            if( link->snoop_by != NULL && get_trust( ch ) >= get_trust( link->snoop_by->character ) ) {
                fFound = TRUE;
                sprintf( buf + strlen( buf ), "%15s : %s\n\r", link->character->real_name( ), link->snoop_by->character->real_name( ) );
            }
    ...
    if( ( victim = one_character( ch, argument, "snoop", (thing_array*) &player_list ) ) == NULL ) {
