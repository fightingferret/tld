...
//ARENA STUFF
    ...
    argtmp = argument;
    //argtmp = one_argument( argtmp );
    if(ch->species != NULL ) 
        if( ch->species->vnum == 2397 ) {
            for( int i = 0; allowed_watch[i] != NULL; i++ ) 
                if ( matches( argtmp, allowed_watch[i] ) ) {
                    watch_valid = TRUE;
                    break;
                }
            if ( !watch_valid ) {
                send ( ch, "You cannot do that right now.\n\r");
                return;
            }
        }
...
bool check_progs( char_data* ch, char* command, char* argument )
{
  ...
  if( check_oprogs( ch, command, argument ) ) 
    return TRUE;
