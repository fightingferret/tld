...
void nanny_intro( link_data* link, char* argument )
{
  bool         suppress_echo;
  pfile_data*          pfile;
// Sadis -- needed to have a tmp buffer thingy, just for kicks
  char    tmp  [ TWO_LINES ];
...
// SADIS -- changed this here to log to info banned account connection
// attempts
  if( is_banned( pfile->account, link ) ) {
    sprintf( tmp, "Connection from banned account %s, from address %s", pfile->account->name, link->host );
    info( "", LEVEL_DEMIGOD, tmp, IFLAG_BAN_INFO, 1 );
    return;
    }
...
/* This section commented since hacking seems to be no longer an issue
 * Uncomment to log all invalid password attempts to mlog of player
    ...
    ch = link->player;
    sprintf( plrlog, "%s -- Invalid password from site: %s", ch->real_name(), link->host );
    player_log( ch, plrlog );
 *
 *  END OF COMMENT
*/
...
/* This section commented since hacking seems to no longer be a problem.
 * Uncomment this section to log address of all passive reconnection
 * attempts to mlog of player
      ch = link->player;
      sprintf( plrlog, "%s -- Passive reconnection from site: %s", ch->real_name(), link->host );
      player_log( ch, plrlog );
 * END OF COMMENT
*/
...
/* This section commented since hacking seems to no longer be a problem
 * Uncomment following section to log all normal logins to mlog of player
  sprintf( plrlog, "%s -- Logging in from site: %s", ch->real_name(), link->host );
  player_log( ch, plrlog );
 * END OF COMMENT
*/
...
/* This section commented since hacking seems to no longer be a problem
 * Uncomment the following to log address of all forced reconnections to
 * mlog of player
    ch = link_prev->player;
    sprintf( buf, "%s -- Forced reconnection from site: %s", ch->real_name(), link_prev->host );
    player_log( ch, buf );
 * END OF COMMENT
*/
