...
{
  char_data *victim;
  if( ch->pcdata != NULL && ch->shdata->skill[ SKILL_PEEK ] == UNLEARNT ) {
    send( ch, "You are untrained in the art of peeking.\n\r" );
    return;
    }
...
// room extras and exits come before mobs/objs in room or
// inventory or worn eq
  if( ( visible = one_visible( ch, argument, "look at",
    (visible_array*) &ch->in_room->extra_descr,
    (visible_array*) ch->array,
    (visible_array*) &ch->contents,
...
void show_room( char_data* ch, room_data* room, bool brief, bool scan )
{
  ...
  if( !brief || !is_set( ch->pcdata->pfile->flags, PLR_BRIEF ) ) {
    if( ( !room->is_dark() || is_set( ch->pcdata->pfile->flags, PLR_HOLYLIGHT ) )
        && !is_set( ch->affected_by, AFF_BLIND ) ) {
/*
    if( !room->is_dark () || is_set( ch->pcdata->pfile->flags, PLR_HOLYLIGHT ) 
        || (!is_set( ch->pcdata->pfile->flags, PLR_HOLYLIGHT ) && !is_set(
ch->affected_by, AFF_BLIND ) ) ) {
*/
...
void room_info( char_data* ch, room_data* room )
{
  ...
  name   = (!ch->in_room->is_dark ()) ? ch->in_room->name : "DARKNESS";
