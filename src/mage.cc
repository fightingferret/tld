...
bool spell_meteor_swarm( char_data* ch, char_data*, void*, int level, int )
{
    ...
    for( int i = *ch->array-1; i >= 0; i-- )
        if( ( rch = character( ch->array->list[i] ) ) != NULL  && rch != ch && can_kill( ch, rch, FALSE ) )
...
bool spell_maelstrom( char_data* ch, char_data*, void*, int level, int )
{
    ...
    for( int i = *ch->array-1; i >= 0; i-- ) 
        if( ( rch = character( ch->array->list[i] ) ) != NULL  && rch != ch && can_kill( ch, rch, FALSE ) )
            damage_physical( rch, ch, spell_damage( SPELL_MAELSTROM, level ), "*The maelstrom" );     
...
void sleep_affect( char_data* ch, char_data* victim, int level, int duration )
{
    ...
    if ( !is_mob( ch ) && ch != NULL && (number_range(0,119) < (2*victim->Save_Magic( ) + victim->Save_Mind( ) + (victim->shdata->level - ch->shdata->level ) - 2*ch->get_skill( SPELL_SLEEP ) ) ) ){
    ...
    if( ch != NULL && is_mob( ch ) && makes_save( victim, ch, RES_MAGIC, SPELL_SLEEP, level ) ) {
        if( victim->position > POS_SLEEPING ) 
            send( victim, "You feel drowsy but quickly shrug it off.\n\r" );
        return;
    }
    ...
    if( victim->mount != NULL ) {
        fsend( *victim->array, "%s tumbles off %s %s is riding and drops to the ground.",
...
bool spell_minor_enchantment( char_data* ch, char_data*, void* vo, int level, int )
{
    ...
    if( obj->value[0] < 0 || obj->value[0] >= 3 ) {
        send( *ch->array, "%s reflects the magical enchantment.\n\r",  obj );
        return TRUE;
    }

    if( roll < 40-level+10*obj->value[0] ) {
        fsend( *ch->array, "%s explodes in a multi-hued burst of light!\n\r", obj );
        obj->Extract( );
        return TRUE;
    } 

    if( roll <= 70-2*level+20*obj->value[0] ) {
        fsend( *ch->array, "%s glows briefly but reflects the enchantment.\n\r", obj );
        return TRUE;
    }

    send( *ch->array, "%s absorbs the magical energy.\n\r", obj );
...
bool spell_major_enchantment( char_data* ch, char_data*, void* vo, int level, int )
{
    ...
    if( roll <= 40-level+20*obj->value[0] ) {
        fsend( *ch->array, "%s glows briefly but reflects the enchantment.\n\r", obj );
        return TRUE;
    }
...
bool spell_identify( char_data* ch, char_data*, void* vo, int, int )
{
    ...
    send( ch, "     Base Cost: %-12d Level: %-11d Weight: %.2f lbs\n\r",  obj->Cost( ), obj_clss->level, obj->Empty_Weight( 1 )/100. );
    sprintf( buf, "            " );

    switch( obj_clss->item_type ) {
        case ITEM_WEAPON :
            sprintf( buf+5,  "   Damage: %dd%d       ", obj_clss->value[1], obj_clss->value[2] );
            sprintf( buf+25, "    Class: %s          ", ( obj_clss->value[3] >= 0 && obj_clss->value[3] < MAX_WEAPON ) ? skill_table[ WEAPON_UNARMED+obj_clss->value[3] ].name : "none" );
            sprintf( buf+45, "   Attack: %s\n\r", ( obj_clss->value[3] >= 0 && obj_clss->value[3] < MAX_WEAPON ) ?
            weapon_attack[ obj_clss->value[3] ] : "none" );
            break;

        case ITEM_WAND :
            sprintf( buf+5,  "    Spell: %s          ", ( obj_clss->value[0] >= 0 && obj_clss->value[0] < MAX_SPELL ) ? spell_table[ obj_clss->value[0] ].name : "none" );
        ...
        case ITEM_TRAP :
            sprintf( buf, "Damage: %dd%d\n\r", obj_clss->value[1], obj_clss->value[2] );
            break;
...
bool spell_blinding_light( char_data* ch, char_data*, void*, int level, int duration )
{
    ...
    sprintf( buf, "*** The room explodes in a flash of light. ***\n\r" );
    if (ch->species == NULL)
        send_color( ch, ANSI_WHITE , buf );
                    
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( rch = character( ch->array->list[i] ) ) != NULL) {
            if( rch == ch || !rch->Can_See( ) || ( rch->species != NULL && !is_set( &rch->species->act_flags, ACT_HAS_EYES ) ) )
                continue;
            if (rch->species == NULL)
                send_color( rch, ANSI_WHITE , buf );
            if( makes_save( rch, ch, RES_MAGIC, SPELL_BLINDING_LIGHT, level ) ) {
                send( rch, "Fortunately you were not looking at the blast and are unaffected.\n\r" );
            }
            else if( !can_kill( ch, rch ) ) {
                send( rch, "Oddly the flash has no affect on you.\n\r" );
            }
            else {
                spell_affect( ch, rch, level, duration, SPELL_BLINDING_LIGHT, AFF_BLIND );
            }
        }
    }
...
bool spell_web( char_data* ch, char_data* victim, void*, int level, int duration )
{
    ...
    if( is_set( victim->affected_by, AFF_ENTANGLED ) ) {
        ...
        if( is_set( victim->affected_by, AFF_FIRE_SHIELD ) ) {
            send( victim, "As the webbing passes through your shield of fire, it flashes and is gone.\n\r" );
            send( *victim->array, "%s grins as the webbing disintegrates against %s fire shield.\n\r", victim, victim->His_Her() );
            return TRUE;
            }

        if( makes_save( victim, ch, RES_DEXTERITY, SPELL_WEB, level ) &&  makes_save( victim, ch, RES_MAGIC, SPELL_WEB, level ) )  {
...
bool spell_eagle_eye( char_data* ch, char_data* victim, void*, int level, int )
{
    ...
    if( victim->in_room->area->status != AREA_OPEN || is_set( &victim->in_room->room_flags, RFLAG_NO_MAGIC ) || ( victim->shdata->level >= LEVEL_BUILDER && get_trust( ch ) < get_trust( victim ) ) ) {
