...
bool target_area_attack( char_data *ch, cast_data* cast, char *argument )
{
  cast->target = ch;
  return TRUE;
}
...
bool target_astral( char_data* ch, cast_data* cast, char* argument )
{
  ...
  for( i = 0; i < MAX_ENTRY_ASTRAL; i++ ) {
    if( fmatches( argument+1, astral_table[i].name ) ) {
...
bool get_target( char_data* ch, cast_data* cast, char* argument )
{
  ...
  switch( type ) {
    ...
    case STYPE_AREA_ATTACK    : return target_area_attack    ( parm );
