...
const char* plr_name [MAX_PLR] = { "Autoassist", "Autoexit",
  "??", "title.color", "??", "Autosplit", "Brief", "Chant",
  ...
  "No.Tell", "Deny", "Freeze", "Edit_Help", "Familiar", "Leader",
  "Enrolled", "Is.Approved", "Donated", "Valid.Email", "Crash.Quit",
  "Avatar.Chnnl", "Build.Chnnl", "Wizinvis", "Holylight", "Logged",
  "Imm.Chnnl" };

const char* act_name [] = { "Is.Npc", "Sentinel", "Scavenger", "Can.Tame",
  "Aggressive", "Stay.Area", "Wimpy", "Humanoid", "Warm_blooded",
  "Summonable", "Assist.Group", "Can.Fly", "Can.Swim", "Mount",
  "Can.Climb", "Can.Carry", "Has.Eyes", "Has.Skeleton", "Ghost",
  "Zero.Regen", "Slithers", "Oozes", "No.Bash", "Mimic", "Predator",
  "Ice.Crystal", "Aggr.Evil", "Aggr.Good", "Carnivore", "Elemental",
  "Use.The", "Night.Only" };
  ...
  if( exact_match( argument, "new" ) ) {
    if( isdigit( *argument ) ) {
      ...
      if( species_copy == NULL ) {
        ...
        species_copy = get_species( 4 );
// SET THIS AS MOB_BLANK IN THE HEADERS -- SADIS 
