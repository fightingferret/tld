...
char* coin_phrase( int* num )
{
    ...
    for( i = MAX_COIN - 1; i >= last; i-- ) {
        if( num[ i ] == 0 )
            continue;
        sprintf( buf + strlen( buf ), "%s %s%d %s", flag ? "," : "", ( i == last && flag ) ? "and " : "", num[i], coin_name[i] );
        flag = TRUE;
    }
...
int get_money( char_data* ch )
{
    ...
    for( int i = 0; i < ch->contents; i++ )
        for( int j = 0; j < 4; j++ )
            if( ( obj = object( ch->contents[i] ) ) != NULL && obj->pIndexData->vnum == coin_vnum[j] ) 
...
void split_money( char_data* ch, int amount, bool msg )
{
    ...
    for( int i = 0; i < *ch->array; i++ ) {
        if( ( gch = character( ch->array->list[i] ) ) != NULL) {
            if( gch != ch && is_same_group( gch, ch ) && ( gch->species == NULL || !is_set( &gch->status, STAT_PET ) ) && gch->Seen( ch ) && gch->position >= POS_RESTING ) {
                members++;
                gch->next_list = group;
                group = gch;
            }
    ...
    for( int d = 0; d < ch->contents; d++ ) {
        if( ( obj = object( ch->contents[d] ) ) != NULL ) {
            for( i = 0; i < 4; i++ ) 
                if( obj->pIndexData->vnum == coin_vnum[i] ) {
                    coin_obj[i] = obj; 
    ...
    for( gch = group; gch != NULL; gch = gch->next_list ) {
        ...
        for( i = MAX_COIN-1; i >= 0; i-- ) {
            ...
            if( coins_split[i] != 0 ) {
                ...
                obj = coin_obj[i];
                for( int tmp = 0; tmp < coins_split[i] ; tmp ++ ) { 
                  obj->From();
                }
                obj = create( get_obj_index( coin_vnum[i] ), coins_split[i]  ); 
                obj->To( gch );                
                found = TRUE;
