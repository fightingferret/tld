...
const char* mprog_trigger[ MAX_MPROG_TRIGGER ] = { "entering",
  ...
  "skin", "timer", "attack", "order", "time", "bash" };
...
//SADIS -- asking sleeping mobs questions is foolish
  if( victim->position <= POS_MEDITATING ) {
    send( ch, "They are in no position to be asked questions.\n\r" );
    return;
    }
