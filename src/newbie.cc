...
{
    send( link, "\n\rWhat you entered was unintelligible.  Please press return and then select\n\rone of the choices given between the left and right brackets.\n\r" );
    press_return( link );
...
bool check_parse_name( link_data* link, char* name )
{
  ...
  /* ALPHA AND PUNCTUATION */
    ...
    if( ispunct( *name ) || ispunct( name[length] ) ) {
        send( link, "\n\rThe first and last letters of a name may not be punctuation.\n\r" );
        return FALSE;
    }
    ...
    for( letter = name; *letter != '\0'; letter++ ) {
        ...
        if( ispunct( *letter ) && *letter != '-' && *letter != '\'' ) {
            send( link, "\n\rNames are not allowed to contain punctuation other than dashes and\n\rapostrophes.\n\r" );
            return FALSE;
        }
...
void nanny_class( link_data* link, char* argument )
{
    ...
    for( i = 0; i < MAX_CLSS; i++ ) 
        if( clss_table[i].open && fmatches( argument, clss_table[i].name ) )
            break;
...
bool allowed_race( char_data* ch, int race )
{
    ...
    for( i = 0; i < MAX_ENTRY_ALIGNMENT; i++ ) 
        if( is_set( &clss_table[ ch->pcdata->clss ].alignments, i ) && is_set( &plyr_race_table[ race ].alignments, i ) ) 
            return TRUE;
...
void nanny_race( link_data* link, char* argument )
{
    ...
    for( i = 0; i < MAX_PLYR_RACE; i++ ) 
        if( plyr_race_table[i].open && ( fmatches( argument, race_table[i].name ) || fmatches( argument, race_table[i].abbrev ) ) ) 
            break;
...
void nanny_stats( link_data* link, char* argument )
{
    ...
    int *stat[] = { &shdata->strength, &shdata->intelligence, &shdata->wisdom, &shdata->dexterity, &shdata->constitution };
    char letter[] = { 'S', 'I', 'W', 'D', 'C' };
    char *name[] = { "Strength", "Intelligence", "Wisdom", "Dexterity", "Constitution" };
    ...
    if( strlen( argument ) < 2 || ( strlen( argument ) < 3 && isdigit( argument[1] ) ) ) {
        ...
        if( i != 5 ) {
            switch( argument[0] ) {
                ...
                case '-' :
                    if( *stat[i]-number < 5 )
                        send( link, "\n\rYou cannot lower your stats below 5.\n\r" );
                    else
                        *stat[i] -= number;
                        break;
                default:
...
{
    return( is_set( &clss_table[ ch->pcdata->clss ].alignments, align )  && is_set( &plyr_race_table[ ch->shdata->race ].alignments, align ) );  
}
...
void nanny_alignment( link_data* link, char* argument )
{
    ...
    for( i = 0; i < MAX_ENTRY_ALIGNMENT; i++ )
        if( !strcasecmp( argument, alignment_table[i].abbrev ) || fmatches( argument, alignment_table[i].name, 3 ) )
            break;
    ...
    send( link, "\n\rA%c %s would not choose to be %s.\n\r", isvowel( *word ), word, alignment_table[i].name );
...
void new_player( player_data* ch )
{
  ...
  /* OPTIONS */
    ...
    set_bit( ch->pcdata->pfile->flags, PLR_ATALK );
  ...
  /* MESSAGES */
    ...
    sprintf( tmp, "%s %s", race_table[ ch->shdata->race ].name, clss_table[ clss ].name );
    ...
    sprintf( tmp, " the %s %s", race_table[ ch->shdata->race ].name, clss_table[ clss ].name );
