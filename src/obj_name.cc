...
const char* campfire_type [] = { "dying", "smoldering", "smoldering", "embering", "embering",
  "crackling", "crackling", "sizzling", "sizzling", "blazing",  "white-hot" }; 
...
void get_obj_adj( char* tmp, char_data* ch, obj_data* obj )
{
  ...
  */
  ...
  if( obj->pIndexData->vnum == 3412 )
    sprintf( tmp+strlen( tmp ), "%s%s", *tmp == '\0' ? "" : ", ",
      campfire_type[ range( 0, obj->value[2], 10 ) ] );
