...
thing_data* cant_take( thing_data* thing, char_data* ch, thing_data* )
{
  ...
  if( !can_wear( obj, ITEM_TAKE ) && is_set( obj->extra_flags, OFLAG_NOSHOW ) )
    return NULL;
  ...
  thing_data* container, const char* msg2, const char* msg3 )
{
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) { 
    if((obj = object( thing )) != NULL)
      check_trigger_oprog(ch, obj, trigger_to_number(msg1));
    if( prev_long )
      send( ch, "\n\r" ); 
    if( !strcmp( msg3, "for" ) ) 
  ...
  if( ( thing = one_shown( *array ) ) != NULL ) {
    ...
    if( ( obj = object( thing ) ) != NULL ) {
      if(is_set(obj->extra_flags, OFLAG_NOREMOVE))
        send(ch, "%s is CURSED!\n\r", obj);
      check_trigger_oprog( ch, obj, OPROG_TRIGGER_WEAR );
      }
    }
  else {
    if( !first_list )
