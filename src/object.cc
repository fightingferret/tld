...
const char* item_type_name [] = { "other", "light", "scroll", "wand",
    ...
    "drink_container", "key", "food", "money", "key.ring", "boat",
    ...
    "fire", "garrote", "camp", "poison", "fuel" };

const char* item_values [] = { "unused", "unused | unused | life time | unused",
    "spell | level | duration | unused", "spell | level | duration | charges",
    "??", "enchantment | damdice | damside | weapon class", "hardness | condition",
    "??", "unused", "enchantment | armor value | bracer ac | unused",
    "spell | level | duration | unused", "charges | unused", "??", "??", "??",
    "capacity | container flags | key vnum | unused", "pick modifier | unused",
    "capacity | contains | liquid | poisoned == -1", "??", "food value | cooked? | unused | poisoned == -1",
    "??", "??", "??", "??", "??", "required to be -1 | unused | liquid | poisoned == -1",
    "range | unused", "trap flags | damdice | damside | unused",
    "unused | unused | lifetime | unused", "unused",
    "unused", "lifetime | unused", "unused | damdice | damside | unused",
    "unused", "unused", "seats | unused", "unused", "unused", "unused", "unused", "unused",
    "timer | next item | unused" };

const char *cont_flag_name [] = { "closeable", "pickproof", "closed", "locked", "unused" };
        ...
        for( custom = shop->custom; custom != NULL; custom = custom->next ) {
            ...
            for( int i = 0; i < MAX_INGRED; i++ ) 
                if( custom->ingred[i] == obj_clss ) {
                    send( ch, "A custom in room %d requires that item as an ingrediant.\n\r", shop->room->vnum );
                    return FALSE;
	            }
        }
...
bool is_same( obj_data* obj1, obj_data* obj2 )
{
    ...
    if( is_empty( obj1->contents ) && !is_empty( obj2->contents ) || !is_empty( obj1->contents ) && is_empty( obj2->contents ) )
        return FALSE;
    ...
    if( obj1->save != NULL ) {
        if( obj2->save == NULL ) {
            obj2->save = obj1->save;
            obj1->save = NULL;
            array      = &obj2->save->save_list;
            for( int i = 0; ; i++ ) {
                if( i >= array->size ) 
                    panic( "Is_Same: Object not in save array." );       
                if( obj1 == array->list[i] ) {
                    array->list[i] = obj2;
                    break;
	            }
            }
        }
        else if( obj2->save != obj1->save )
            return FALSE;
    }
...
{
    return( owner == NULL || ( ch != NULL && ch->pcdata != NULL && ch->pcdata->pfile == owner ) );
}
...
void set_owner( pfile_data* pfile, thing_array& array )
{
    ...
    for( int i = 0; i < array; i++ ) {
        if( ( obj = object( array[i] ) ) != NULL ) {
            if( obj->owner == NULL && obj->pIndexData->item_type != ITEM_MONEY )
                obj->owner = pfile; 
...
void set_owner( obj_data* obj, char_data* buyer, char_data* seller )
{
    ...
    if( obj->Belongs( seller ) && obj->pIndexData->item_type != ITEM_MONEY )
        obj->owner = ( ( buyer != NULL && buyer->pcdata != NULL ) ? buyer->pcdata->pfile : NULL );
...
{
    const char* abbrev [] = { "wls", "dmg", "vwn", "wrn", "vsc", "scr", "rea", "goo", "vgo", "exc" };
    ...
    sprintf( tmp, "%s%s%s", i > 4 ? ( i > 7 ? blue( ch ) : green( ch ) ) : ( i > 2 ? yellow( ch ) : red( ch ) ), abbrev[i], normal( ch ) );
...
const char* obj_data :: condition_name( char_data* ch, bool ansi )
{
    ...
    sprintf( tmp, "%s%s%s", i > 4 ? ( i > 7 ? blue( ch ) : green( ch ) ) : ( i > 2 ? yellow( ch ) : red( ch ) ), txt, normal( ch ) );
...
int Obj_Data :: Cost( )
{
    ...
    if( ( pIndexData->item_type == ITEM_WEAPON || pIndexData->item_type == ITEM_ARMOR ) && is_set( extra_flags, OFLAG_IDENTIFIED ) ) {
        if( value[0] < 0 )
            return 0;
...
void enchant_object( obj_data* obj )
{
    ...
    if( ( obj->pIndexData->item_type == ITEM_WEAPON || obj->pIndexData->item_type == ITEM_ARMOR )
        ...
        if( ( i = number_range( 0, 1000 ) ) >= 900 ) {
            obj->value[0] = ( i > 950 ? ( i > 990 ? ( i == 1000 ? 3 : 2 ) : 1 ) : ( i < 910 ? ( i == 900 ? -3 : -2 ) : -1 ) );
...
void set_alloy( obj_data* obj, int level )
{
    ...
    if( metal > MAT_ADAMANTINE ) {
        metal = MAT_BRONZE;
        for( ; ; ) {
            if( metal == MAT_ADAMANTINE || number_range( 0, level+75 ) > level-10*(metal-MAT_BRONZE) )
                break;
            metal++;
        }
...
obj_data* create( obj_clss_data* obj_clss, int number )
{
    ...
    if( obj_clss->item_type != ITEM_ARMOR && obj_clss->item_type != ITEM_WEAPON
        && !strcmp( obj_clss->before, obj_clss->after ) && obj_clss->plural[0] != '{' && obj_clss->singular[0] != '{' )
...
void load_objects( void )
{
    ...
    for( ; ; ) {
        ...
        //New field read, compare string read to spell_table names, value[0] inherits that int
        //This allows us to dynamically change this field depending on order of spells in abil.h
        
        tmp                     = fread_string( fp, MEM_OBJ_CLSS );
        if ( obj_clss->item_type == ITEM_SCROLL || obj_clss->item_type == ITEM_WAND || obj_clss->item_type == ITEM_POTION )
            for ( int i = 0 ; i < MAX_SKILL ; i++ )
                if ( fmatches( tmp, skill_table[i].name ) ) {
                    obj_clss->value[0] = i - SPELL_FIRST;
                    break;
                }
