...
bool check_oprogs( char_data *ch, char *command, char *argument )
{
  oprog_data *oprog;
  obj_data   *obj;
  int        i;

  for(i=0; i < ch->in_room->contents; i++) {
    if( ( obj = object(ch->in_room->contents[i])) == NULL)
      continue;
 
    for(oprog = obj->pIndexData->oprog; oprog != NULL; oprog = oprog->next)
      if(member( command, oprog->command, TRUE ) &&
         member( argument, oprog->target ) )
         break;
...
void check_trigger_oprog( char_data *ch, obj_data *obj, int trigger ) {

  oprog_data *oprog;
  
  for(oprog = obj->pIndexData->oprog; oprog != NULL; oprog = oprog->next)
    if( oprog->trigger == trigger ) {
      disrupt_spell( ch );
      var_ch = ch;
      var_room = ch->in_room;
      var_obj = obj;
...
int trigger_to_number( char *msg )
{
  int i;
  for(i = 0; i < MAX_OPROG_TRIGGER; i++)
    if(!strcmp(msg, oprog_trigger[i]))
      return i;
  return 15;
}
