...
// Sadis -- new iflag type: ban_info
const char *iflag_name [] = { "auction", "info.clans", "deaths", "levels", "logins", "info.bugs", "requests", "writes","arena", "ban_info", "note" };
...
{
    if( ( info->clan != NULL && info->clan != ch->pcdata->pfile->clan ) || !can_see_info( ch, info->pfile, info->type ) )
        return empty_string;
...
void do_info( char_data* ch, char* argument )
{
    ...
    if( toggle( ch, argument, "Info channel", ch->pcdata->pfile->flags, PLR_INFO ) ) 
        return;
...
{
    char*        bug_name  [] = { "## Roach ##", "## Beetle ##", "## Aphid ##" };
    ...
    switch ( flag ) {
        case IFLAG_AUCTION:
            sprintf( istring, "Auction" );
            break;
        case IFLAG_ARENA:
            sprintf( istring, "Arena" );
            break;
        case IFLAG_BUGS:
            sprintf( istring, "%s", bug_name[priority-1] );
            break;
        default:
            sprintf( istring, "Info" );
    }
    
    sprintf( tmp1, "%s || %%s", istring );
    for( link = link_list; link != NULL; link = link->next ) {
        if( link->connected != CON_PLAYING || ( victim = link->player ) == NULL || victim == ch
            || !is_set( victim->pcdata->pfile->flags, PLR_INFO ) || ( clan != NULL && victim->pcdata->pfile->clan != clan ) 
    ...
    if( ch != NULL && ( flag == IFLAG_LOGINS || flag == IFLAG_DEATHS || flag == IFLAG_LEVELS ) ) {
        if( setting == 0 || ( setting == 1 && !victim->Befriended( ch ) ) || ( setting == 2 && !victim->Recognizes( ch ) ) 
    ...
    for( ; ; ) {
        ...
        switch ( flag ) {
            case IFLAG_AUCTION:
                send_color( link->character, COLOR_AUCTION,  tmp1, tmp2 );
                break;
            case IFLAG_ARENA:
                send_color( link->character, COLOR_ARENA,  tmp1, tmp2 );
                break;
            case IFLAG_BUGS:
                send_color( link->character, COLOR_INFO,  tmp1, tmp2 );
                break;
            default:
                send_color( link->character, COLOR_INFO,  tmp1, tmp2 );
        }
...
void do_message( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        display_flags( "*Message", &message_flags[0], &message_flags[1], &ch->pcdata->message, MAX_MESSAGE, ch );
        page( ch, "\n\r" );
        display_levels( "Message", &mess_settings[0], &mess_settings[1], &ch->pcdata->mess_settings, MAX_MESS_SETTING, ch );
        page( ch, "\n\r" );
        page_centered( ch, "[ Also see the iflag and option commands ]" );
        return;
    }

    if( set_flags( &message_flags[0], &message_flags[1], &ch->pcdata->message, MAX_MESSAGE, NULL, ch, argument, FALSE, FALSE ) != NULL ) 
        return;

    if( !set_levels( &mess_settings[0], &mess_settings[1], &ch->pcdata->mess_settings, MAX_MESS_SETTING, ch, argument, FALSE ) )
        send( ch, "Unknown option.\n\r" );
}
...
void do_options( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        display_flags( "*Option Flags", &plr_name[0], &plr_name[1], ch->pcdata->pfile->flags, MAX_PLR_OPTION, ch );
        page( ch, "\n\r" );
        display_levels( "Option", &plyr_settings[0], &plyr_settings[1], &ch->pcdata->pfile->settings, MAX_SETTING, ch );
        page( ch, "\n\r" );
    ...
    for( i = 0; i < MAX_PLR_OPTION; i++ ) {
        if( matches( argument, plr_name[i] ) ) {
            ...
            send( ch, "%s set to %s.\n\r", plr_name[i], true_false( ch->pcdata->pfile->flags, i ) );
    ...
    if( !set_levels( &plyr_settings[0], &plyr_settings[1], &ch->pcdata->pfile->settings, MAX_SETTING, ch, argument, FALSE ) )
