...
void update_high_death( char_data* ch );
...
void add_list( pfile_data**& list, int& size, pfile_data* pfile )
{
    ...
    else if( &list == &high_death ) {
        pos = search( high_death, max_death, pfile, compare_death );
    }
    else {
...
void remove_list( pfile_data**& list, int& size, pfile_data* pfile )
{
    ...
    if( &list == &site_list ) {
        ...
        if( pos < 0 || strcasecmp( site_list[pos]->last_host, pfile->last_host ) ) {
...
pfile_data* find_pfile( const char* name, char_data* ch )
{
    ...
    if( -i-1 < max_pfile && !strncasecmp( pfile_list[-i-1]->name, name, strlen( name ) ) )
...
{
    for( int i = 0; i < player_list; i++ )
        if( player_list[i]->pcdata->pfile == pfile && player_list[i]->in_room != NULL )
...
void forced_quit( player_data* ch, bool crash )
{
    ...
    if( link != NULL ) {
        ...
        reset_screen( ch );
        send( link, "Thank you for visiting Lyon's Den.\n\r" );
    }
    ...
    for( link_new = link_list; link_new != NULL; link_new = link_next ) {
        ...
        if( link_new->character != NULL && link_new != link && !strcmp( link_new->character->descr->name, ch->descr->name ) ) {
...
void do_delete( char_data* ch, char* argument )
{
    ...
    if( ch->shdata->level > 5 && ch->pcdata->trust < LEVEL_BUILDER ) {
        send( ch, "No way. Your over level 5, you shouldn't go around deleting yourself.\n\r" );
        return;
    }
    for( int i = 0; i < auction_list; i++ ) 
        if( auction_list[i]->seller == ch->pcdata->pfile ) {
            send( ch, "Are you an idiot?! You have stuff on auction.\n\r" );
            return;
        }
    ...
    send_seen( ch, "%s throws %sself on %s sword.\n\r", ch, ch->Him_Her( ), ch->His_Her( ) );
    send_seen( ch, "A giant hand decends from the sky and takes %s's spirit.\n\r", ch );
...
void do_save( char_data* ch, char* )
{
    ...
    if( is_mob( ch ) )
        return;
    //For High Death, is modify_pfile( ch ) better here?
    ch->pcdata->pfile->deaths = ch->shdata->deaths;   
    pc = (player_data*) ch;
...
void do_afk( char_data* ch, char* argument )
{
    ...
    if( is_mob( ch ) )
        return;
    if( !get_flags( ch, argument, &flags, "m", "Message" ) )
        return;

    if( is_set( &ch->status, STAT_AFK ) ) {
        remove_bit( &ch->status, STAT_AFK );
        send( ch, "You are no longer AFK.\n\r" );
        return;
    }

    if( *argument == '\0' ) {
        send( ch, "You are now AFK\n\r" );
        free_string( ch->pcdata->afk_message, MEM_PLAYER );
        ch->pcdata->afk_message = alloc_string( "Your message was delivered, but I am currently afk.", MEM_PLAYER );
    ...
    free_string( ch->pcdata->afk_message, MEM_PLAYER );
    ch->pcdata->afk_message = alloc_string( tmp, MEM_PLAYER );

    send( ch, "You are now afk, and your messages has been set.\n\r" );
    set_bit( &ch->status, STAT_AFK );
    return;
}
...
void do_password( char_data* ch, char* argument )
{
    ...
    if( is_mob( ch ) || !get_flags( ch, argument, &flags, "a", "Password" ) )
        return;
...
const char* high_score_name( int i, char_data* ch ) {
    ...
    return( is_incognito( high_score[i], ch ) ? "???" : high_score[i]->name );
}

const char* high_death_name( int i, char_data* ch ) {
    ...
    sprintf( tmp, "%s (%d) ", high_death[i]->name, high_death[i]->deaths );   
    return( is_incognito( high_death[i], ch ) ? "???" : tmp );
}
...
void do_high( char_data* ch, char* argument )
{
    ...
    if( isalpha( *argument ) ) {
        ...
        if( matches( argument, "death" ) ) {
            high_death = NULL;
            update_high_death( ch );
            c1 = max( atoi( argument ), 0 );
            c1 = min( c1, max_death );
            send( ch, "\n\r    -=-=-=-=-=-=-=-=-=-=-=HIGH  DEATH=-=-=-=-=-=-=-=-=-=-=-=-   \n\r");
            for( i = c1; i < c1+10 ; i++ ) {
                sprintf( r1,  "[%d]", i + 1 );                
                sprintf( r2,  "[%d]", i + 11 );
                sprintf( tmp, "%10s  %-20s %10s  %s\n\r", r1, high_death_name( i , ch ), r2, high_death_name( 10+i, ch ) );
                send( tmp, ch );                
            }
            send( ch,     "    -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-   \n\r\n\r");
            high_death = NULL;
        ...
        if( max( atoi( argument ) , 1) != 1){
            send( ch, "Illegal syntax - see help high score.\n\r" );
            return;
        }

        c1 = 1; /*max( atoi( argument ), 1 );*/

        for( i = c2 = 0; c2-c1 < 19 && i < max_high; i++ )
        if( ( clss == MAX_CLSS || high_score[i]->clss == clss ) && ( race == MAX_PLYR_RACE || high_score[i]->race == race ) && ++c2 >= c1 )
            who[c2-c1] = i;
    ...
    for( i = 0; i < 10; i++ ) {
        sprintf( r1,  "[%d]", c1+i );
        sprintf( r2,  "[%d]", c2+i );
        sprintf( tmp, "%10s  %-20s %10s  %s\n\r", r1, high_score_name( clss == -1 ? c1+i-1 : who[i], ch ), r2, high_score_name( clss == -1 ? c2+i-1 : who[10+i], ch ) );
        send( tmp, ch );
    }
    send( ch,     "    -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-    \n\r\n\r");
...
void update_high_death( char_data* ch )
{
    ...
    for( int i = 0; i < max_pfile; i++ ) {
        pfile = pfile_list[i];
        if ( pfile->level > 20 && pfile->level < 91 )
            add_list( high_death, max_death, pfile );
    }
}
