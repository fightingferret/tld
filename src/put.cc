...
void put_in_pipe    ( char_data*, thing_array*, obj_data* );
...
void do_put( char_data* ch, char* argument )
{
    ...
    if( contains_word( argument, "on", arg ) ) {
        if( ( victim = one_character (ch, argument, "put on", ch->array)) != NULL ) {
            if ((item = one_object (ch, arg, "put on", &ch->contents)) == NULL) {
                send( ch, "You aren't holding anything like that.\n\r" );
            }
            else {
                put_obj_on( ch, victim, item );
	        }
        }
        return;
    }
  ...
  /* PIPES */
    ...
    if( container->pIndexData->item_type == ITEM_PIPE ) {
        item = (obj_data*) array->list[0]; 
        if( array->size > 1 ) {
            send( ch, "You can only put one item in a pipe at a time.\n\r" );
        }
        else if( !is_empty(container->contents) ) {
            send( ch, "%s already contains %s.\n\r", container, list_name( ch, &container->contents) );
        }
        else if( item->pIndexData->item_type != ITEM_TOBACCO ) {
            send( ch, "%s is not something you wish to smoke.\n\r", item->Seen_Name( ch ) );
        } 
        else 
            put_in_pipe(ch, array, container);
        return;
    }    

  /* BAGS & KEYRINGS */

    if( container->pIndexData->item_type != ITEM_CONTAINER && container->pIndexData->item_type != ITEM_KEYRING ) {
        fsend( ch, "You can only put items in containers and keyrings and %s is neither.", container );
        return;
    }
...
thing_data* keys( thing_data* thing, char_data*, thing_data* container )
{
    obj_data *obj = (obj_data*) thing;
    obj_data *cont = (obj_data*) container;

    if(cont->pIndexData->item_type == ITEM_KEYRING && obj->pIndexData->item_type != ITEM_KEY)
        return NULL;
...
void put_in_pipe( char_data* ch, thing_array* array, obj_data* container )
{
    thing_array    subset  [ 3 ];
    thing_func*      func  [ 3 ]  = { same, cursed, put };

    sort_objects( ch, *array, container, 3, subset, func );
    ...
    page_publ( ch, &subset[2], "put", container, "into", container->array == ch->array ? " on the ground" : empty_string );
...
void put_in( char_data* ch, thing_array* array, obj_data* container )
{
    ...
    thing_func*      func  [ 5 ]  = { same, cursed, keys, wont_fit, put };
    ...
    page_priv( ch, &subset[0], "can't fold", NULL, "into itself" );
    page_priv( ch, &subset[1], "can't let go of" );
    page_priv( ch, &subset[2], "can't attach", container, "onto", container->array == ch->array ? " on the ground" : empty_string );
    page_priv( ch, &subset[3], "doesn't have room for", container );
...
void wear_obj( char_data* victim, obj_data* obj, char_data* ch )
{
    ...
    if( is_set( &victim->species->wear_part, WEAR_HORSE_BACK ) && can_wear( obj, ITEM_WEAR_HORSE_BACK ) ) {
        if( get_eq_char( victim, WEAR_HORSE_BACK, 0 ) != NULL ) {
    ...
    else if( is_set( &victim->species->wear_part, WEAR_HORSE_BODY ) && can_wear( obj, ITEM_WEAR_HORSE_BODY ) ) {
        if( get_eq_char( victim, WEAR_HORSE_BODY, 0 ) != NULL ) {
            send( ch, "%s is already wearing something to protect %s body.\n\r", victim, NULL, NULL, ch );
            return;
        }
        loc = WEAR_HORSE_BODY;
        send( ch, "You put %s on %s, protecting %s body.\n\r", obj, victim, victim->His_Her( ) );
        send_seen( ch, "%s puts %s on %s, protecting %s body.\n\r", ch, obj, victim, victim->His_Her( ) ); 
    }

    else if( is_set( &victim->species->wear_part, WEAR_HORSE_FEET ) && can_wear( obj, ITEM_WEAR_HORSE_FEET ) ) {
        if( get_eq_char( victim, WEAR_HORSE_FEET, 0 ) != NULL ) {
