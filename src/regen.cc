...
void gain_condition( char_data* ch, int iCond, int value )
{
    ...
    if( value < 0 ) {
        if( iCond == COND_FULL && new_cond < 0 && is_set( &ch->pcdata->message, MSG_HUNGER ) ) {
            sprintf( buf, "You are %s\n\r", new_cond < -20 ? ( new_cond < -30 ? "starving to death!" : "ravenous." ) : ( new_cond < -10
                ? "hungry." : "mildly hungry." ) );
            send( buf, ch );
        }
        if( iCond == COND_THIRST && new_cond < 0 && is_set( &ch->pcdata->message, MSG_THIRST ) ) {
            sprintf( buf, "You%s\n\r", new_cond < -20 ? ( new_cond < -30 ? " are dying of thirst!" : "r throat is parched." ) : ( new_cond
...
void regenerate( char_data* ch )
{
    if( ch->hit < ch->max_hit && !is_set( ch->affected_by, AFF_POISON ) ) {
        ...
        if( ch->hit >= ch->max_hit ) {
            ...
            if( ch->pcdata != NULL && is_set( &ch->pcdata->message, MSG_MAX_HIT ) )
                send( ch, "You are at full health.\n\r" );
        }
        update_max_move( ch );
    }

    if( ch->mana < ch->max_mana || ( ch->position == POS_MEDITATING && ch->hit < ch->max_hit ) ) {
        ch->mana += tenth( ch->Mana_Regen( ) );
        if( ch->mana >= ch->max_mana ) {
            ch->mana = ch->max_mana;
            if( ch->pcdata != NULL && is_set( &ch->pcdata->message, MSG_MAX_ENERGY ) )
                send( ch, "You are at full energy.\n\r" );
        }
    }
    ...
    if( ch->move < ch->max_move ) {
        ...
        if( ch->move >= ch->max_move ) {
            ch->move = ch->max_move;
            if( ch->pcdata != NULL  && is_set( &ch->pcdata->message, MSG_MAX_MOVE ) )
                send( ch, "You are at full stamina.\n\r" );
        }
    }
        ...
        if( !is_empty( ch->leech_list ) ) {
            if( ch != ch->leech_list[0]->victim ) {
                fsend( ch, "You are unable to continue supporting the %s affect on %s.",
                    aff_char_table[ch->leech_list[0]->type].name, ch->leech_list[0]->victim );
                fsend( ch->leech_list[0]->victim, "The energy supply to the %s affect on you is cut.",
...
int Char_Data :: Hit_Regen( )
{
    ...
    if( species == NULL ) {
        ...
        if( position == POS_MEDITATING && mana != 0 ) {
            regen += regen*( shdata->skill[ SKILL_MEDITATE ] +shdata->skill[ SKILL_TRANCE ] )/15;
        }
    }      
    else {
    ...
    if( is_set( affected_by, AFF_CAMPING ) ) {
        if ( ( campfire = find_type( *array, ITEM_CAMP ) ) == NULL) {
            strip_affect( this, AFF_CAMPING );
            return regen/2;
        }
        ...
        regen += 2*(3*campfire->value[2]);
    } 
  
    //if( in_room->sector_type == SECT_UNDERWATER && !is_set( affected_by, AFF_WATER_BREATHING ) )
        //regen = min( regen, 0 );
