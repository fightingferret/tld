...
void reset_room( room_data* room )
{
  ...
  set_bit( &room->room_flags, RFLAG_RESET3 );
  set_bit( &room->room_flags, RFLAG_RESET4 );
  set_bit( &room->room_flags, RFLAG_RESET5 );
   ...
   mob_data*& mob, char_data*& leader )
{
  ...
  if( is_set( &mob->species->act_flags, ACT_NIGHT_ONLY ) &&
      ( weather.hour >= 5 && weather.hour <= 20 ) ) {
      mob->lurking_in = room;
      mob->From();
      mob->To( get_room_index(ROOM_LURKER) );
      }
