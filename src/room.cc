...
const char* rflag_name[ MAX_RFLAG ] = { "Lit", "Safe", "Indoors", "No.Mob",
  ...
  "No.Mount", "Arena", "Donation", "No.Pkill", "Reset3", "Reset4",
  "Reset5", "No.Night.Only", "No.Respawn.Undead" };
...
bool Room_Data :: is_dark( )
{
    ...
    if (is_set (&room_flags, RFLAG_LIT)) 
        return FALSE;
    if (isday( ) /*sunlight*/ && (!is_set (&room_flags, RFLAG_INDOORS))) 
        return FALSE;
    if (isday( ) && (!is_set (&room_flags, RFLAG_UNDERGROUND))) 
        return FALSE;
