...
bool load_char( link_data* link, char* name, const char* dir )
{
  ...
  if( !read_char( fp, imm, ch ) ) {
    ...
    roach( "-- Name: %s", name );
    if( strcmp( dir, PLAYER_DIR ) || ( fp = fopen( tmp, "r" ) ) == NULL 
      || !read_char( fp, imm, ch ) ) 
...
int reference( player_data* ch, thing_array& obj_list, int i )
{
  obj_array*  array  = (obj_array*) &obj_list;
  obj_data*   obj;
...
void save_objects( )
{
  ...
  for( i = 0; i < MAX_OBJ_INDEX; i++ ) {
    ...
    fprintf( fp, "%d %d %d %d\n", obj_clss->value[0], obj_clss->value[1], obj_clss->value[2], obj_clss->value[3] );

/// Save spell name in relation to integer value[0] for items that cast    
    
    if ( obj_clss->item_type == ITEM_SCROLL || obj_clss->item_type == ITEM_WAND || obj_clss->item_type == ITEM_POTION )
        fprintf( fp, "%s~\n", skill_table[obj_clss->value[0] + SPELL_FIRST ].name );
    else 
        fprintf( fp, "~\n" );
///
