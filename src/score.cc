...
char* condition_name( char_data* ch )
{
    ...
    sprintf( buf, "%s%s%s%s", IS_DRUNK( ch ) ? "drunk" : "sober", ch->pcdata->condition[COND_THIRST] < 0 ? " thirsty" : "", ch->pcdata->condition[COND_FULL] < 0 ? " hungry" : "", IS_AFFECTED( ch, AFF_POISON ) ? " poisoned" : "" );
  return buf;
}
...
void do_identity( char_data* ch, char* )
{
    ...
    if( ch->species == NULL ) 
        page( ch, "         Class: %-13s Race: %-15s ID#: %d\n\r", clss_table[ pcdata->clss ].name, race_table[ ch->shdata->race ].name, pcdata->pfile->ident );
    else
        page( ch, "          Race: %s\n\r",  race_table[ ch->shdata->race ].name );

        page( ch, "           Sex: %-8s Alignment: %s\n\r", sex_name[ch->sex], alignment_table[ ch->shdata->alignment ].name );

    if( pc != NULL ) {
        page( ch, "      Religion: %-12s Piety: %-14s Fame: %s\n\r", religion_table[ pcdata->religion ].name, lookup( piety_index, pcdata->piety ), lookup( fame_index, ch->shdata->fame ) );
        page( ch, "        Deaths: %-7d Recog Char: %d\n\r", ch->shdata->deaths, pcdata->recognize == NULL ? 0 : pcdata->recognize->size );
//        page( ch, "        Height: %-11s Weight: %.2f lbs\n\n\r", "??", (float) ch->Weight( )/100 );
    ...
    if( has_permission( ch, PERM_GOTO ) ) {
        page( ch, "         Bamfin: %s\n\r", imm->bamfin == empty_string ? "none" : imm->bamfin );
        page( ch, "        Bamfout: %s\n\r", imm->bamfout == empty_string ? "none" : imm->bamfout );
    }
...
void do_score( char_data* ch, char* )
{
    ...
    sprintf( buf+25, "   Weight: %d/%d       ", ch->contents.weight/100, ch->Capacity( )/100 );
    ...
    if( pc != NULL ) 
        page( ch, "      Qst_Pnts: %-8d Prac_Pnts: %-9d Gsp_Pnts: %d\n\r", pcdata->quest_pts, pcdata->practice, pc->gossip_pts );

    page( ch, "     Hit Regen: %-8.1f Ene Regen: %-9.1f Mv Regen: %.1f\n\r", float( ch->Hit_Regen( )/10. ), float( ch->Mana_Regen( )/10. ), float( ch->Move_Regen( )/10. ) );
    ...
    if( ( wield2 = ch->Wearing( WEAR_HELD_L ) ) == NULL ) {
        ...
        page( ch, "     Hitroll: %+3.2f/%+3.2f      Damroll: %+3.2f/%+3.2f\n\r\n\r", ch->Hitroll( wield ), ch->Hitroll( wield2 ), ch->Damroll( wield ), ch->Damroll( wield2 ) );
    }

  /*  ABILITIES */

    sprintf( buf+5, "Str: %2d(%2d)  Int: %2d(%2d)  Wis: %2d(%2d)", ch->Strength( ),     ch->shdata->strength, ch->Intelligence( ), ch->shdata->intelligence, ch->Wisdom( ),       ch->shdata->wisdom );
    sprintf( buf+strlen( buf ), "  Dex: %2d(%2d)  Con: %2d(%2d)\n\r\n\r", ch->Dexterity( ),    ch->shdata->dexterity, ch->Constitution( ), ch->shdata->constitution );
    page( ch, buf );

  /*  RESISTANCES */

    const char* name [] = { "Magic", "Fire", "Cold", "Mind", "Electric", "Acid", "Poison" }; 
    ...
    const char* color [] = { "",  red( ch ), blue( ch ), "", yellow( ch ), green( ch ), "" };

    for( i = 0; i < 7; i++ ) {
        sprintf( tmp+40*i, "%10s", "" );
        sprintf( tmp+40*i+9-strlen( name[i] ), "%s%s: %+d%%%s%s", color[i], name[i], resist[i], normal( ch ), abs( resist[i] ) > 99 ? ""  : ( abs( resist[i] ) > 9 ? " " : "  " ) ); 
    }
  ...
  /*  MONEY, POSITION, CONDITION */
    ...
    page_centered( ch, "[Also try the command identity for more information.]" ); 
...
void do_statistics( char_data* ch, char* argument )
{
    ...
    sprintf( tmp, "%20s%20s%20s\n\r", "Hit Points", "Energy", "Move" );
    page_underlined( ch, tmp );

    page( ch, "%13s:%7d%13s:%7d%13s:%7d\n\r", "Base", victim->base_hit, "Base", victim->base_mana, "Base", victim->base_move );
    page( ch, "%13s:%7d)%12s:%7d)%12s:%7d)\n\r", "(Exp", (int) victim->Mean_Hp( ), "(Exp", (int) victim->Mean_Mana( ), "(Exp", (int) victim->Mean_Move( ) );
    page( ch, "%13s:%7d%13s:%7d%13s:%7d\n\r", "+Mod", victim->mod_hit, "+Mod", victim->mod_mana, "+Mod", victim->mod_move );
    page( ch, "%13s:%7d%13s:%7d%13s:%7d\n\r", "+Con", victim->shdata->level*(victim->Constitution( )-12)/2, "+Int", victim->shdata->level*victim->Intelligence( )/4, "Max",  victim->max_move );

    page( ch, "%13s:%7d%13s:%7d\n\r", "Max",    victim->max_hit, "-Leech", leech_max( victim ) );
    page( ch, "%34s:%7d\n\r", "-Prep", prep_max( victim ) );

    page( ch, "%34s:%7d\n\r", "Max", victim->max_mana );
}  
