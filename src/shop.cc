...
void shop_update( void )
{
    ...
    for( int i = 0; i < mob_list; i++ ) {
        ...
        if( keeper->pShop == NULL || keeper->reset == NULL || player_in_room( keeper->in_room ) )
            continue;
...
int get_cost( char_data* keeper, char_data* ch, obj_data* obj, bool fBuy )
{
    ...
    if( !fBuy ) {
        if( obj->pIndexData->item_type == ITEM_ARMOR || obj->pIndexData->item_type == ITEM_WEAPON )
            cost /= 12;
        else
            cost /= 4;
    }
...
int repair_cost( char_data* keeper, obj_data* obj )
{
    ...
    if( !will_trade( keeper, obj ) || obj->pIndexData->repair > keeper->pShop->repair )
        return 0;
...
{
    if( pet->species == NULL || pet->leader != NULL || !is_set( &pet->species->act_flags, ACT_CAN_TAME ) )
        return FALSE;
...
void do_list( char_data* ch, char* argument )
{
    ...
    if( is_set( &ch->in_room->room_flags, RFLAG_PET_SHOP ) ) {
        ...
        for( int i = 0; i < *array; i++ ) {
            ...
            send( ch, "%-25s%8d%8d\n\r", pet->Seen_Name( ch, 1, TRUE ), pet->species->price, pet->shdata->level );
        }
    ...
    for( int i = 0; i < *array; i++ ) {
        ...
        page( ch, "%-37s%8d%8s%8d%5s%-s\n\r", truncate( obj->Seen_Name( ch, 1, TRUE ), 37 ), obj->temp, level,
...
thing_data* uninterested( thing_data* thing, char_data* ch, thing_data* keeper )
{
    ...
    if( ( obj->pIndexData->item_type == ITEM_WEAPON || obj->pIndexData->item_type == ITEM_ARMOR ) && obj->value[0] <= -1 )
        return NULL;
...
void do_sell( char_data* ch, char* argument )
{
    ...
    if( ( array = several_things( ch, argument, "sell", &ch->contents ) ) == NULL ) 
        return;
...
void do_value( char_data* ch, char* argument )
{
    ...
    if( !obj->Belongs( ch ) ) {
        sprintf( buf, "%s is stolen so I would never buy it.", obj->Seen_Name( ch ) );
    ...
    if( obj->Damaged( ) ) {
        if( rcost > 0 ) 
            sprintf( buf, "I see %s is damaged.  I can repair it for %d cp %s ", obj->Seen_Name( ch ), rcost, cost > 0 ? "or" : "but" ); 
        else 
            sprintf( buf, "I see %s is damaged.  I am unable to repair it %s ",  obj->Seen_Name( ch ), cost > 0 ? "but" : "and" );
        if( cost > 0 && ( (obj->pIndexData->item_type == ITEM_ARMOR || obj->pIndexData->item_type == ITEM_WEAPON ) ? obj->value[0] >=0 : 1 ) )
...
void do_repair( char_data* ch, char* argument )
{
    ...
    if( ( cond = repair_condition( obj ) ) < 0 ) {
        process_tell( keeper, ch, "That item is too old to be worth repairing." );
        return;
    }

    if( !obj->Damaged( ) ) {
        process_tell( keeper, ch, "That isn't damaged enough to be worth repairing." );
        return;
    }
...
//Sadis didn't like seeing himself get things repaired.
    for( int k = 0 ; k < ch->in_room->contents ; k++ ) {
        if( ( ( rch = player( ch->in_room->contents[k] ) ) != NULL ) && rch != ch && !is_set( rch->affected_by, AFF_BLIND ) && rch->position > POS_SLEEPING ) {
            fsend( rch, "%s gets %s repaired.", ch->Seen_Name( rch ), obj->Seen_Name( rch ) );
        }
    }
...
void do_melt( char_data *ch, char *argument )
{
    ...
    if( keeper->pShop->materials & ( 1 << metal ) == 0 ) {
        process_tell( keeper, ch, "Sorry that is not made out of a metal I deal with." );
        return;
    }
    ...
    for( int k = 0 ; k < ch->in_room->contents ; k++ ) {
        if( ( ( tch = player( ch->in_room->contents[k] ) ) != NULL ) && tch != ch && !is_set( tch->affected_by, AFF_BLIND ) && tch->position > POS_SLEEPING ) {
            fsend( tch, "%s has %s melted down.", ch->Seen_Name( tch ), 
            obj->Seen_Name( tch ) );
        }
    }
    ...
    length = strlen( buf );
    sprintf( &buf[length], "%s then pulls it out and after much hammering and reheating hands you %s.",
...
void do_shedit( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        ...
        sprintf( buf, "Shop Keeper: %s  [ Vnum: %d ]\n\r\n\r", ( species == NULL ? "none" : species->descr->name ), shop->keeper );
...
#define types 3
    ...
    const char** name1 [types] = { &shop_flags[0], &item_type_name[0], &material_name[0] };
    const char** name2 [types] = { &shop_flags[1], &item_type_name[1], &material_name[1] };

    int*    flag_value [types] = { &shop->flags, shop->buy_type, &shop->materials };
    int      uses_flag [types] = { 1, 1, 1 };

    response = flag_handler( title, name1, name2, flag_value, max, uses_flag, (const char*) NULL, ch, argument, types );
...
char_data* active_shop( char_data* ch )
{
    ...
    for( int i = 0; i < room->contents; i++ )
        if( ( keeper = mob( room->contents[i] ) ) != NULL && keeper->pShop != NULL && keeper->pShop->room == room  && IS_AWAKE( keeper ) && ch->Seen( keeper ) )
            return keeper;
...
void save_shops( )
{
    ...
    for( shop = shop_list; shop != NULL; shop = shop->next ) {
        ...
        fprintf( fp, "%5d %5d %5d ", shop->flags, shop->buy_type[0], shop->buy_type[1] );
        fprintf( fp, "%2d %5d %5d %5d\n", shop->repair, shop->materials, shop->open_hour, shop->close_hour );
