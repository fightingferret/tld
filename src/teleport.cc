...
void make_gate( room_data* from, room_data* to, int time )
{
    obj_data* gate;
...
bool spell_astral_gate( char_data* ch, char_data*, void* vo, int, int )
{
    ...
    time = 50 + ( ch->get_skill( SPELL_ASTRAL_GATE ) * number_range( 10, 20 ) );

    make_gate( room, ch->in_room, time );
    make_gate( ch->in_room, room, time );
...
bool spell_blink( char_data* ch, char_data* victim, void*, int level, int )
{
    ...
    for( int i = 0; i < level; i++ ) {
        if( ( exit = random_open_exit( room ) ) == NULL )
            break;
        if( ch->array->where != exit->to_room && exit->to_room->area->status == AREA_OPEN )
            room = exit->to_room;
    }
...
bool spell_summon( char_data* ch, char_data* victim, void*, int, int )
{
    ...
    if( victim->shdata->level >= LEVEL_BUILDER || victim->species != NULL || is_set( &victim->in_room->room_flags, RFLAG_NO_RECALL ) ) {
    ...
    if( is_set( victim->pcdata->pfile->flags, PLR_NO_SUMMON ) ) {
        send( ch, "%s has prevented the summoning.\n\r", victim );
        return TRUE;
    }
  ...
  /* MAKE LIST */
    ...
    for( int i = 0; i < victim->in_room->contents; i++ ) 
        if( ( rch = character( victim->in_room->contents[i] ) ) != NULL && rch->leader == victim && rch->species != NULL ) 
            list += rch;
  ...
  /* TRANSFER CHARACTERS */
    ...
    if (victim->position != POS_SLEEPING && !is_set( victim->affected_by, AFF_BLIND ) ) {
