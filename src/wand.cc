...
void do_zap( char_data* ch, char* argument )
{
  ...
  if( ( wand = one_object( ch, arg, "zap", &ch->contents, &ch->wearing ) ) == NULL )
    return;
...
void do_quaff( char_data* ch, char* argument )
{
  ...
  ( *spell_table[ spell ].function )( ch, ch, NULL,
    level, duration );
...
// Sadis -- just for kicks
  if( spell == SPELL_RECALL-SPELL_FIRST && ch->fighting != NULL )
    stop_fight( ch );
