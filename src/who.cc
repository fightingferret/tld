...
void display_last( pfile_data* pfile, char_data* ch )
{
    page( ch, "%-15s %s  %s\n\r", pfile->name, ltime( pfile->last_on ), is_demigod( ch ) ? pfile->last_host : "" );
}
...
void do_whois( char_data* ch, char* argument )
{
    ...
    if( *argument == '\0' ) {
        send( ch, "Specify full name of character.\n\rTo search for a name use last <first letter of name>.\n\r" );
        return;
    }
    ...
    if( !is_incognito( pfile, ch ) ) {
        ...
        if( pfile->level < LEVEL_APPRENTICE )
            send( ch, "       Level: %d  [ Rank %d%s ]\n\r", pfile->level, pfile->rank+1, number_suffix( pfile->rank+1 ) );
        else
    ...
    if( pfile->level < LEVEL_APPRENTICE )
        send( ch, "      Bounty: %d\n\r", pfile->bounty );
    if( !is_incognito( pfile, ch ) ) 
        send( ch, "        Clan: %s\n\r\n\r", ( pfile->clan == NULL || !knows_members( ch, pfile->clan ) ) ? "none" : pfile->clan->name );
    ...
    if( see_account || see_private ) {
        send( ch, "        Site: %s\n\r", pfile->last_host );
        send( ch, "     Account: %s\n\r", pfile->account == NULL ? "none" : pfile->account->name );
        if( is_god( ch ) && pfile->account != NULL ) 
            send( ch, "  Acnt. Pswd: %s\n\r", pfile->account->pwd );
    }

    if( pfile->account != NULL && ( ( email = is_set( pfile->flags, PLR_EMAIL_PUBLIC ) ) || see_private ) ) 
        send( ch, "       Email: %s%s\n\r", pfile->account->email, email ? "" : "  (Hidden)" );

    send( ch, "    Homepage: %s\n\r", pfile->homepage == empty_string ? "none" : pfile->homepage );

    if( see_private ) {
        send( ch, "\n\r" );
        send( ch, scroll_line[1] );
        send( ch, "\n\r" );
        send( ch, "     Balance: $%.2f\n\r", pfile->account == NULL ? 0. : (float)pfile->account->balance/100 );
    }
...
void do_qwho( char_data* ch, char* argument )
{
    ...
    for( link = link_list; link != NULL; link = link->next ) {
        ...
        if( !can_see_who( ch, wch ) || ( flags == 1 && !ch->Recognizes( wch ) ) || ( flags == 2 && !ch->Befriended( wch ) ) || !fmatches( argument, ch->descr->name, -1 ) ) 
            continue;      
        ...
        if( ch->pcdata->terminal != TERM_ANSI ) {
            ...
            sprintf( tmp, "%s%17s%s%s", same_clan( ch, wch ) ? red( ch ) : ( ch->Befriended( wch ) ? green( ch ) : ( ch->Recognizes( wch ) ? yellow( ch ) : "" ) ), wch->descr->name, normal( ch ), ++i%4 ? "" : "\n\r" );
            page( ch, tmp );
        }
    }
        ...
        *buf = '\0';
        ...
        for( link = link_list; link != NULL; link = link->next ) {
            ...
            if( wch->shdata->level >= LEVEL_APPRENTICE ) {
                ...
                if( clan != NULL && ( title = get_title( wch->pcdata->pfile ) ) != NULL && knows_members( ch, clan ) ) {
                    abbrev = "CL";
	        }
            ...
            if( abbrev == "AV" )
                ...
                sprintf( buf, "{   Incognito   }" );
                }
                else {
                    sprintf( buf, "{%2d %s %s %s}", wch->shdata->level, clss_table[wch->pcdata->clss].abbrev, race_table[wch->shdata->race].abbrev,clan != NULL && knows_members( ch, clan ) ? clan->abbrev : " -- ");
                }
            else
                if( is_incognito( wch, ch ) ) {
                    sprintf( buf, "[   Incognito   ]" );
        ...
        if ( is_set( ch->pcdata->pfile->flags, PLR_TITLE_COLOR ) )
            convert_to_ansi( ch, wch->pcdata->title , tmp1 );
        else
            remove_ansi_codes( wch->pcdata->title , tmp1 );             
        
        if( type > 0 ) {
            sprintf( tmp, " %%s%%s%%s%%-%ds\n\r", 53-strlen( wch->descr->name )  );
            sprintf( buf+strlen( buf ), tmp, bold_cyan_v( ch ), wch->descr->name, normal( ch ), tmp1 );
        }
        else {
            if( imm->wizinvis > 0 && is_set( wch->pcdata->pfile->flags, PLR_WIZINVIS ) )
                sprintf( tmp, " %%s%%s%%s%%-%ds   %d\n\r", 53-strlen( wch->descr->name ) , imm->wizinvis );
            else  
                sprintf( tmp, " %%s%%s%%s%%-%ds\n\r", 53-strlen( wch->descr->name ) );
...
void w3_who( )
{
    ...
    for( int i = 0; i < player_list; i++ ) {
        pc = player_list[i]; 
        if( pc->In_Game( ) && !is_set( pc->pcdata->pfile->flags, PLR_WIZINVIS ) )
            fprintf( fp, "%s %s <br>\n", race_table[ pc->shdata->race ].abbrev, pc->descr->name );
    }
...
void do_users( char_data* ch, char* argument )
{
    ...
    for( link = link_list; link != NULL; link = link->next ) {
        victim = link->player;
        if( ( is_god( ch ) || ( victim != NULL && can_see_who( ch, victim ) ) ) && !strncasecmp( argument, victim == NULL ? "" : victim->descr->name, length ) ) {
            if( flags == 0 ) {
                sprintf( tmp, "%-15s   %-30s   %s", victim == NULL ? "(Logging In)" : victim->descr->name, is_demigod( ch ) || victim == ch || ( victim != NULL && victim->shdata->level < LEVEL_APPRENTICE ) ? &link->host[ max( 0, strlen( link->host )-30 ) ] : "(protected)", ( victim == NULL || victim->array == NULL ) ? "(nowhere)" : victim->array->where->Location( ) );
                truncate( tmp, 78 );
                strcat( tmp, "\n\r" );
            }
        else {
            ...
            sprintf( tmp, "%-15s   %s\n\r", victim->descr->name, victim->descr->singular );          
        }
        else if( is_set( &flags, 2 ) ) {
            ...
            sprintf( tmp, "%-18s%3s %3s  %2s %3d   %3d %3d   %4d %4d %4d   %-7d\n\r", victim->descr->name, clss_table[victim->pcdata->clss].abbrev, race_table[victim->shdata->race].abbrev, alignment_table[ victim->shdata->alignment ].abbrev, victim->shdata->level, victim->pcdata->trust, (int) current_time-victim->timer, victim->max_hit, victim->max_mana, victim->max_move, victim->bank );
        }
        }
        page( ch, tmp );
        count++;
